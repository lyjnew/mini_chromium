...
scoped_ptr<Value> Value::CreateNullValue() {
  return make_crbase_scoped_ptr(new Value(TYPE_NULL));
}
...
Value* Value::DeepCopy() const {
  ...
  CR_DCHECK(IsType(TYPE_NULL));
  return CreateNullValue().release();
}

scoped_ptr<Value> Value::CreateDeepCopy() const {
  return make_crbase_scoped_ptr(DeepCopy());
}
...
StringValue::StringValue(const std::string& in_value)
    : Value(TYPE_STRING),
      value_(in_value) {
  CR_DCHECK(IsStringUTF8(in_value));
}
...
scoped_ptr<DictionaryValue> DictionaryValue::From(scoped_ptr<Value> value) {
  ...
  if (value && value->GetAsDictionary(&out)) {
    ...
    return make_crbase_scoped_ptr(out);
  }
  return nullptr;
}
...
bool DictionaryValue::HasKey(const std::string& key) const {
  CR_DCHECK(IsStringUTF8(key));
  ValueMap::const_iterator current_entry = dictionary_.find(key);
  CR_DCHECK((current_entry == dictionary_.end()) || current_entry->second);
...
void DictionaryValue::Set(const std::string& path, Value* in_value) {
  Set(path, make_crbase_scoped_ptr(in_value));
}
...
void DictionaryValue::SetWithoutPathExpansion(const std::string& key,
  ...
  if (!ins_res.second) {
    CR_DCHECK_NE(ins_res.first->second, bare_ptr);  // This would be bogus
...
void DictionaryValue::SetWithoutPathExpansion(const std::string& key,
                                              Value* in_value) {
  SetWithoutPathExpansion(key, make_crbase_scoped_ptr(in_value));
}
...
bool ListValue::Set(size_t index, Value* in_value) {
  ...
  if (index >= list_.size()) {
    ...
    CR_DCHECK(list_[index] != in_value);
    delete list_[index];
