...
int RandInt(int min, int max) {
  ...
  int result =
      static_cast<int>(min + 
                      static_cast<int64_t>(crbase::RandGenerator(range)));
  CR_DCHECK_GE(result, min);
  CR_DCHECK_LE(result, max);
  return result;
}

double RandDouble() {
  return BitsToOpenEndedUnitInterval(crbase::RandUint64());
}
...
double BitsToOpenEndedUnitInterval(uint64_t bits) {
  ...
  CR_DCHECK_GE(result, 0.0);
  CR_DCHECK_LT(result, 1.0);
  return result;
}

uint64_t RandGenerator(uint64_t range) {
  CR_DCHECK_GT(range, 0u);
