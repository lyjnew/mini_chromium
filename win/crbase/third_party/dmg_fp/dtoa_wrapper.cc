...
static crbase::LazyInstance<crbase::Lock>::Leaky
    dtoa_lock_0 = CR_LAZY_INSTANCE_INITIALIZER;
static crbase::LazyInstance<crbase::Lock>::Leaky
    dtoa_lock_1 = CR_LAZY_INSTANCE_INITIALIZER;
...
inline static void ACQUIRE_DTOA_LOCK(size_t n) {
  CR_DCHECK(n < 2);
  crbase::Lock* lock = n == 0 ? dtoa_lock_0.Pointer() : dtoa_lock_1.Pointer();
  lock->Acquire();
}
...
#include "crbase/third_party/dmg_fp/dtoa.cc"