  ...
  if ((g_logging_destination & LOG_TO_FILE) != 0) {
    ...
    if (g_log_file == INVALID_HANDLE_VALUE || g_log_file == nullptr) {
      ...
      *g_log_file_name = file_path.Append(
          CR_FILE_PATH_LITERAL("debug.log")).value();
...
bool CrBaseInitLoggingImpl(const LoggingSettings& settings) {
  ///crbase::CommandLine* command_line = crbase::CommandLine::ForCurrentProcess();
...
int GetVlogLevelHelper(const char* file, size_t N) {
  ...
  ///return vlog_info ?
  ///    vlog_info->GetVlogLevel(crbase::StringPiece(file, N - 1)) :
  ///    GetVlogVerbosity();
...
LogMessage::~LogMessage() {
  ...
  if ((g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0) {
    ...
    cr_ignore_result(fwrite(str_newline.data(), str_newline.size(), 1, stderr));
    fflush(stderr);
...
void RawLog(int level, const char* message) {
  if (level >= g_min_log_level) {
    ...
    if (message_len > 0 && message[message_len - 1] != '\n') {
      do {
        ///rv = CR_HANDLE_EINTR(write(STDERR_FILENO, "\n", 1));
