...
///#include "crbase/base_switches.h"
...
///#include "crbase/third_party/valgrind/memcheck.h"
...
inline bool IsProfilerTimingEnabled() {
  ...
  ///  if (!crbase::CommandLine::InitializedForCurrentProcess())
  ///    return true;
  ///  current_timing_enabled =
  ///      (crbase::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
  ///           switches::kProfilerTiming) ==
  ///       switches::kProfilerTimingDisabledValue)
  ///          ? DISABLED_TIMING
  ///          : ENABLED_TIMING;
  ///  crbase::subtle::NoBarrier_Store(&g_profiler_timing_enabled,
  ///                                  current_timing_enabled);
  ///}
...
DeathData::DeathData(const DeathData& other)
  ...
  CR_DCHECK(other.last_phase_snapshot_ == nullptr);
}
...
ThreadData::ThreadData(const std::string& suggested_name)
  ...
  CR_DCHECK_GE(suggested_name.size(), 0u);
...
ThreadData* ThreadData::Get() {
  ...
  if (!worker_thread_data) {
    CR_DCHECK_GT(worker_thread_number, 0);
    worker_thread_data = new ThreadData(worker_thread_number);
  }
  CR_DCHECK_GT(worker_thread_data->worker_thread_number_, 0);
...
void ThreadData::OnThreadTermination(void* thread_data) {
  CR_DCHECK(thread_data);  // TLS should *never* call us with a NULL.
...
void ThreadData::OnThreadTerminationCleanup() {
  ...
  CR_DCHECK_EQ(this->next_retired_worker_, reinterpret_cast<ThreadData*>(NULL));
...
void ThreadData::Initialize() {
  ...
  if (!tls_index_.initialized()) {  // Testing may have initialized this.
    ...
    CR_DCHECK_EQ(crbase::subtle::NoBarrier_Load(&status_), DORMANT_DURING_TESTS);
  }
...
void ThreadData::InitializeAndSetTrackingStatus(Status status) {
  CR_DCHECK_GE(status, DEACTIVATED);
  CR_DCHECK_LE(status, PROFILING_ACTIVE);
...
ThreadData::Status ThreadData::status() {
  return 
      static_cast<ThreadData::Status>(crbase::subtle::Acquire_Load(&status_));
}
...
void ThreadData::ShutdownSingleThreadedCleanup(bool leak) {
  ...
  ThreadData* thread_data_list;
  {
    ...
    while (first_retired_worker_) {
      ...
      CR_CHECK_GT(worker->worker_thread_number_, 0);
...
TaskStopwatch::~TaskStopwatch() {
#if CR_DCHECK_IS_ON()
  CR_DCHECK(state_ != RUNNING);
  CR_DCHECK(child_ == NULL);
#endif
}

void TaskStopwatch::Start() {
#if CR_DCHECK_IS_ON()
  CR_DCHECK(state_ == CREATED);
  state_ = RUNNING;
#endif
...
#if CR_DCHECK_IS_ON()
  if (parent_) {
    CR_DCHECK(parent_->state_ == RUNNING);
    CR_DCHECK(parent_->child_ == NULL);
    parent_->child_ = this;
  }
#endif
...
void TaskStopwatch::Stop() {
  ...
  CR_DCHECK(current_thread_data_->current_stopwatch_ == this);
