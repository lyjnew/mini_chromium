...
#ifndef MINI_CHROMIUM_CRBASE_COMMAND_LINE_H_
#define MINI_CHROMIUM_CRBASE_COMMAND_LINE_H_
...
class CRBASE_EXPORT CommandLine {
 public:
  ...
  typedef crbase::string16 StringType;

  typedef StringType::value_type CharType;
  typedef std::vector<StringType> StringVector;
  typedef std::map<std::string, StringType> SwitchMap;
  typedef std::map<crbase::StringPiece, const StringType*> StringPieceSwitchMap;
  ...
  static CommandLine FromString(const crbase::string16& command_line);
  ...
  // StringPiece.
  bool HasSwitch(const crbase::StringPiece& switch_string) const;
  bool HasSwitch(const char switch_constant[]) const;

  // Returns the value associated with the given switch. If the switch has no
  // value or isn't present, this method returns the empty string.
  // Switch names must be lowercase.
  std::string GetSwitchValueASCII(const crbase::StringPiece& switch_string) const;
  FilePath GetSwitchValuePath(const crbase::StringPiece& switch_string) const;
  StringType GetSwitchValueNative(const crbase::StringPiece& switch_string) const;
  ...
  void ParseFromString(const crbase::string16& command_line);
...
#endif  // MINI_CHROMIUM_CRBASE_COMMAND_LINE_H_