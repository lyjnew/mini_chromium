...
bool InitializeSymbols() {
  ...
  if (!SymInitialize(GetCurrentProcess(), NULL, TRUE)) {
    ...
    // process).
    CR_DLOG(ERROR) << "SymInitialize failed: " << g_init_error;
    return false;
  }
  ...
  if (!SymGetSearchPathW(GetCurrentProcess(),
                         ...
                         kSymbolsArraySize)) {
    g_init_error = GetLastError();
    CR_DLOG(WARNING) << "SymGetSearchPath failed: " << g_init_error;
    return false;
  }
  ...
  if (!SymSetSearchPathW(GetCurrentProcess(), new_path.c_str())) {
    g_init_error = GetLastError();
    CR_DLOG(WARNING) << "SymSetSearchPath failed." << g_init_error;
    return false;
  }
...
class SymbolContext {
 ...
 private:
  ...
  crbase::Lock lock_;
  CR_DISALLOW_COPY_AND_ASSIGN(SymbolContext);
};
...
StackTrace::StackTrace() {
  ...
  count_ = CaptureStackBackTrace(0, cr_arraysize(trace_), trace_, NULL);
}
...
void StackTrace::InitTrace(const CONTEXT* context_record) {
  ...
  while (StackWalk64(machine_type,
                     ...
                     &SymGetModuleBase64,
                     NULL) &&
         count_ < cr_arraysize(trace_)) {
    trace_[count_++] = reinterpret_cast<void*>(stack_frame.AddrPC.Offset);
  }

  for (size_t i = count_; i < cr_arraysize(trace_); ++i)
    trace_[i] = NULL;
}
