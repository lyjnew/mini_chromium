...
#ifndef MINI_CHROMIUM_CRBASE_DEBUG_PROFILER_H_
#define MINI_CHROMIUM_CRBASE_DEBUG_PROFILER_H_
...
CRBASE_EXPORT void StartProfiling(const std::string& name);

// Stop profiling and write out data.
CRBASE_EXPORT void StopProfiling();

// Force data to be written to file.
CRBASE_EXPORT void FlushProfiling();

// Returns true if process is being profiled.
CRBASE_EXPORT bool BeingProfiled();

// Reset profiling after a fork, which disables timers.
CRBASE_EXPORT void RestartProfilingAfterFork();

// Returns true iff this executable is instrumented with the Syzygy profiler.
CRBASE_EXPORT bool IsBinaryInstrumented();
...
CRBASE_EXPORT AddDynamicSymbol GetProfilerAddDynamicSymbolFunc();
CRBASE_EXPORT MoveDynamicSymbol GetProfilerMoveDynamicSymbolFunc();

}  // namespace debug
}  // namespace crbase

#endif  // MINI_CHROMIUM_CRBASE_DEBUG_PROFILER_H_