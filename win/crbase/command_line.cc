...
const CommandLine::CharType kSwitchTerminator[] = CR_FILE_PATH_LITERAL("--");
const CommandLine::CharType kSwitchValueSeparator[] = CR_FILE_PATH_LITERAL("=");
...
void CommandLine::set_slash_is_not_a_switch() {
  ...
  CR_DCHECK_EQ(wcscmp(
                   kSwitchPrefixes[cr_arraysize(kSwitchPrefixes) - 1], L"/"), 0);
...
bool CommandLine::HasSwitch(
    const crbase::StringPiece& switch_string) const {
...
bool CommandLine::HasSwitch(const char switch_constant[]) const {
  return HasSwitch(crbase::StringPiece(switch_constant));
}
...
std::string CommandLine::GetSwitchValueASCII(
  ...
  if (!IsStringASCII(value)) {
    CR_DLOG(WARNING) << "Value of switch (" 
                     << switch_string << ") must be ASCII.";
    return std::string();
  }
...
void CommandLine::PrependWrapper(const CommandLine::StringType& wrapper) {
  ...
  StringVector wrapper_argv = SplitString(
      wrapper, FilePath::StringType(1, ' '), crbase::TRIM_WHITESPACE,
...
CommandLine::StringType CommandLine::GetCommandLineStringInternal(
  ...
  if (!params.empty()) {
    string.append(StringType(CR_FILE_PATH_LITERAL(" ")));
...
CommandLine::StringType CommandLine::GetArgumentsStringInternal(
  ...
  for (size_t i = 1; i < argv_.size(); ++i) {
    ...
    parse_switches &= arg != kSwitchTerminator;
    if (i > 1)
      params.append(StringType(CR_FILE_PATH_LITERAL(" ")));
    if (parse_switches && IsSwitch(arg, &switch_string, &switch_value)) {
