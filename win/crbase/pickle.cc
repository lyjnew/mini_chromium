...
Pickle::Pickle(int header_size)
    : header_(NULL),
      ...
      capacity_after_header_(0),
      write_offset_(0) {
  CR_DCHECK_GE(static_cast<size_t>(header_size), sizeof(Header));
  CR_DCHECK_LE(header_size, kPayloadUnit);
  Resize(kPayloadUnit);
  header_->payload_size = 0;
}
...
void Pickle::Resize(size_t new_capacity) {
  CR_CHECK_NE(capacity_after_header_, kCapacityReadOnly);
...
bool Pickle::PeekNext(size_t header_size,
                      ...
                      const char* end,
                      size_t* pickle_size) {
  CR_DCHECK_EQ(header_size, bits::Align(header_size, sizeof(uint32_t)));
  CR_DCHECK_GE(header_size, sizeof(Header));
  CR_DCHECK_LE(header_size, static_cast<size_t>(kPayloadUnit));
...
inline void* Pickle::ClaimUninitializedBytesInternal(size_t length) {
  CR_DCHECK_NE(kCapacityReadOnly, capacity_after_header_)
...
inline void Pickle::WriteBytesCommon(const void* data, size_t length) {
  CR_DCHECK_NE(kCapacityReadOnly, capacity_after_header_)
      << "oops: pickle is readonly";
  CR_MSAN_CHECK_MEM_IS_INITIALIZED(data, length);
