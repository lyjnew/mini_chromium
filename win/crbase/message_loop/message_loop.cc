...
///#include "crbase/metrics/statistics_recorder.h"
#include "crbase/run_loop.h"
///#include "crbase/third_party/dynamic_annotations/dynamic_annotations.h"
...
#define CRMESSAGE_PUMP_UI \
    crbase::scoped_ptr<MessagePump>(new crbase::MessagePumpForUI())

#define CR_MESSAGE_PUMP_DEFAULT \
    crbase::scoped_ptr<MessagePump>(new crbase::MessagePumpDefault())

  if (type == MessageLoop::TYPE_UI) {
    if (message_pump_for_ui_factory_)
      return message_pump_for_ui_factory_();
    return CRMESSAGE_PUMP_UI;
  }
  if (type == MessageLoop::TYPE_IO)
    return scoped_ptr<MessagePump>(new MessagePumpForIO());

  CR_DCHECK_EQ(MessageLoop::TYPE_DEFAULT, type);
  return CR_MESSAGE_PUMP_DEFAULT;
}
...
scoped_ptr<MessageLoop> MessageLoop::CreateUnbound(
  ...
  return make_crbase_scoped_ptr(new MessageLoop(type, pump_factory));
}
...
void MessageLoop::DeleteSoonInternal(
    const crtracked_objects::Location& from_here,
