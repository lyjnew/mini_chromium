...
void MessagePumpForUI::WaitForWork() {
  ...
  CR_DCHECK_NE(WAIT_FAILED, result) << GetLastError();
}
...
bool MessagePumpForUI::ProcessPumpReplacementMessage() {
  ...
  CR_DCHECK(!have_message || kMsgHaveWork != msg.message ||
            msg.hwnd != message_hwnd_);

  // Since we discarded a kMsgHaveWork message, we must update the flag.
  int old_have_work = InterlockedExchange(&have_work_, 0);
  CR_DCHECK(old_have_work);
...
void MessagePumpForIO::WaitForWork() {
  ...
  // recursion problems.
  CR_DCHECK_EQ(1, state_->run_depth) << "Cannot nest an IO message loop!";
...
bool MessagePumpForIO::ProcessInternalIOItem(const IOItem& item) {
  if (reinterpret_cast<void*>(this) == reinterpret_cast<void*>(item.context) &&
    ...
    CR_DCHECK(!item.bytes_transfered);
    InterlockedExchange(&have_work_, 0);
    return true;
  }
  return false;
...
void MessagePumpForIO::WillProcessIOEvent() {
  CR_FOR_EACH_OBSERVER(IOObserver, io_observers_, WillProcessIOEvent());
}

void MessagePumpForIO::DidProcessIOEvent() {
  CR_FOR_EACH_OBSERVER(IOObserver, io_observers_, DidProcessIOEvent());
}
