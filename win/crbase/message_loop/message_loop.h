...
#ifndef MINI_CHROMIUM_CRBASE_MESSAGE_LOOP_MESSAGE_LOOP_H_
#define MINI_CHROMIUM_CRBASE_MESSAGE_LOOP_MESSAGE_LOOP_H_
...
#include "crbase/sequenced_task_runner_helpers.h"
...
class CRBASE_EXPORT MessageLoop : public MessagePump::Delegate {
 public:
  ...
  void PostTask(const crtracked_objects::Location& from_here,
                const Closure& task);

  void PostDelayedTask(const crtracked_objects::Location& from_here,
                       const Closure& task,
                       TimeDelta delay);

  void PostNonNestableTask(const crtracked_objects::Location& from_here,
                           const Closure& task);

  void PostNonNestableDelayedTask(const crtracked_objects::Location& from_here,
                                  const Closure& task,
                                  TimeDelta delay);
  ...
  // on the thread that executes MessageLoop::Run().
  template <class T>
  void DeleteSoon(const crtracked_objects::Location& from_here, 
                  const T* object) {
 ...
 private:
  ...
  template <class T, class R> friend class 
  crbase::subtle::DeleteHelperInternal;
  template <class T, class R> friend class 
  crbase::subtle::ReleaseHelperInternal;

  void DeleteSoonInternal(const crtracked_objects::Location& from_here,
                          void(*deleter)(const void*),
                          const void* object);
  void ReleaseSoonInternal(const crtracked_objects::Location& from_here,
                           void(*releaser)(const void*),
                           const void* object);

  CR_DISALLOW_COPY_AND_ASSIGN(MessageLoop);
};
...
#endif  // MINI_CHROMIUM_CRBASE_MESSAGE_LOOP_MESSAGE_LOOP_H_