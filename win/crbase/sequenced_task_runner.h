...
#ifndef MINI_CHROMIUM_CRBASE_SEQUENCED_TASK_RUNNER_H_
#define MINI_CHROMIUM_CRBASE_SEQUENCED_TASK_RUNNER_H_
...
class CRBASE_EXPORT SequencedTaskRunner : public TaskRunner {
 public:
  ...
  bool PostNonNestableTask(const crtracked_objects::Location& from_here,
                           const Closure& task);

  virtual bool PostNonNestableDelayedTask(
      const crtracked_objects::Location& from_here,
      const Closure& task,
      crbase::TimeDelta delay) = 0;

  // Submits a non-nestable task to delete the given object.  Returns
  // true if the object may be deleted at some point in the future,
  // and false if the object definitely will not be deleted.
  template <class T>
  bool DeleteSoon(const crtracked_objects::Location& from_here,
                  const T* object) {
    return
  ...
  template <class T>
  bool ReleaseSoon(const crtracked_objects::Location& from_here,
                   T* object) {
    return
 ...
 private:
  ...
  bool DeleteSoonInternal(const crtracked_objects::Location& from_here,
                          void(*deleter)(const void*),
                          const void* object);

  bool ReleaseSoonInternal(const crtracked_objects::Location& from_here,
...
#endif  // MINI_CHROMIUM_CRBASE_SEQUENCED_TASK_RUNNER_H_