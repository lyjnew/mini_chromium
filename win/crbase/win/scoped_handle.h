...
#ifndef MINI_CHROMIUM_CRBASE_WIN_SCOPED_HANDLE_H_
#define MINI_CHROMIUM_CRBASE_WIN_SCOPED_HANDLE_H_
...
class GenericScopedHandle {
  CR_MOVE_ONLY_TYPE_FOR_CPP_03(GenericScopedHandle)
...
class HandleTraits {
 ...
 private:
  CR_DISALLOW_IMPLICIT_CONSTRUCTORS(HandleTraits);
};
...
class DummyVerifierTraits {
 ...
 private:
  CR_DISALLOW_IMPLICIT_CONSTRUCTORS(DummyVerifierTraits);
};

// Performs actual run-time tracking.
class CRBASE_EXPORT VerifierTraits {
 public:
  typedef HANDLE Handle;
 ...
 private:
  CR_DISALLOW_IMPLICIT_CONSTRUCTORS(VerifierTraits);
};
...
// for ScopedHandle.
void CRBASE_EXPORT DisableHandleVerifier();

// This should be called whenever the OS is closing a handle, if extended
// verification of improper handle closing is desired. If |handle| is being
// tracked by the handle verifier and ScopedHandle is not the one closing it,
// a CHECK is generated.
void CRBASE_EXPORT OnHandleBeingClosed(HANDLE handle);

}  // namespace win
}  // namespace crbase

#endif  // MINI_CHROMIUM_CRBASE_WIN_SCOPED_HANDLE_H_