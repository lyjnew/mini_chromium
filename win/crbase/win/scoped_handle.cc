...
struct HandleHash {
  size_t operator()(const HANDLE& handle) const {
    ...
    return crbase::Hash(buffer, sizeof(buffer));
  }
};
...
typedef crbase::internal::LockImpl NativeLock;
crbase::LazyInstance<NativeLock>::Leaky g_lock = CR_LAZY_INSTANCE_INITIALIZER;
...
class AutoNativeLock {
 ...
 private:
  NativeLock& lock_;
  CR_DISALLOW_COPY_AND_ASSIGN(AutoNativeLock);
};
...
class ActiveVerifier {
 ...
 private:
  ...
  NativeLock* lock_;
  HandleMap map_;
  CR_DISALLOW_COPY_AND_ASSIGN(ActiveVerifier);
};
