...
///  CR_DISALLOW_COPY_AND_ASSIGN(Watcher);
///};
//
...
LONG RegKey::GetValueNameAt(int index, std::wstring* name) const {
  wchar_t buf[256];
  DWORD bufsize = cr_arraysize(buf);
...
void RegistryValueIterator::Initialize(HKEY root_key,
                                       const wchar_t* folder_key,
                                       REGSAM wow64access) {
  CR_DCHECK_EQ(wow64access & ~kWow64AccessMask, static_cast<REGSAM>(0));
  LONG result =
...
bool RegistryValueIterator::Read() {
  if (Valid()) {
    ...
    if (result == ERROR_SUCCESS) {
      CR_DCHECK_LT(to_wchar_size(value_size_), value_.size());
...
bool RegistryKeyIterator::Read() {
  if (Valid()) {
    DWORD ncount = cr_arraysize(name_);
    FILETIME written;
