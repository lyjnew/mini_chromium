...
#ifndef MINI_CHROMIUM_CRBASE_WIN_SCOPED_COM_INITIALIZER_H_
#define MINI_CHROMIUM_CRBASE_WIN_SCOPED_COM_INITIALIZER_H_
...
class ScopedCOMInitializer {
 public:
  ...
  ~ScopedCOMInitializer() {
    ...
    CR_DCHECK_EQ(GetCurrentThreadId(), thread_id_);
#endif
    if (succeeded())
      CoUninitialize();
  }
 ...
 private:
...
#ifndef NDEBUG
    if (hr_ == S_FALSE)
      CR_LOG(ERROR) << "Multiple CoInitialize() calls for thread " 
                    << thread_id_;
    else
      CR_DCHECK_NE(RPC_E_CHANGED_MODE, hr_) 
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(ScopedCOMInitializer);
};

}  // namespace win
}  // namespace crbase

#endif  // MINI_CHROMIUM_CRBASE_WIN_SCOPED_COM_INITIALIZER_H_