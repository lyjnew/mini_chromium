...
bool CheckAndDuplicateHandle(HANDLE source, ScopedHandle* target) {
  ...
  if (crbase::win::GetVersion() == crbase::win::VERSION_WIN8 ||
      crbase::win::GetVersion() == crbase::win::VERSION_WIN8_1) {
    ...
    if (nt_duplicate_object != NULL) {
      ...
      if (status < 0) {
        CR_DPLOG(ERROR) << "Failed to duplicate a handle.";
...
bool ScopedProcessInformation::DuplicateFrom(
  ...
  CR_DCHECK(!IsValid()) << "target ScopedProcessInformation must be NULL";
  CR_DCHECK(other.IsValid()) << "source ScopedProcessInformation must be valid";
