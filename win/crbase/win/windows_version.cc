  ...
  if ((version_number_.major == 5) && (version_number_.minor > 0)) {
    ...
    switch (version_number_.minor) {
      ...
      case 2:
        ...
        break;
      default:
        CR_DCHECK_EQ(version_number_.minor, 3);
        version_ = VERSION_WIN8_1;
        break;
    }
  } else if (version_number_.major == 10) {
    version_ = VERSION_WIN10;
  } else if (version_number_.major == 11) {
    version_ = VERSION_WIN11;
  } else if (version_number_.major > 6) {
    CR_NOTREACHED();
...
std::string OSInfo::processor_model_name() {
  if (processor_model_name_.empty()) {
    ...
    crbase::win::RegKey key(HKEY_LOCAL_MACHINE, kProcessorNameString, KEY_READ);
    string16 value;
...
OSInfo::WOW64Status OSInfo::GetWOW64StatusForProcess(HANDLE process_handle) {
  ...
  CR_CHECK(is_wow64_process != NULL)
      << "Failed to get produce 'IsWow64Process'.";
