...
class LazyIsUser32AndGdi32Available {
 ...
 private:
  static bool IsWin32kSyscallsDisabled() {
    ...
    if (crbase::win::GetVersion() < crbase::win::VERSION_WIN8)
      return false;
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(LazyIsUser32AndGdi32Available);
};
...
bool SetAppIdForPropertyStore(IPropertyStore* property_store,
  ...
  CR_DCHECK(lstrlen(app_id) < 64 && wcschr(app_id, L' ') == NULL);
...
bool DisplayVirtualKeyboard() {
  ...
  static LazyInstance<string16>::Leaky osk_path = CR_LAZY_INSTANCE_INITIALIZER;
  ...
  if (osk_path.Get().empty()) {
    ...
    if (common_program_files_offset != string16::npos) {
      ...
      if (buffer_size) {
        ...
        CR_DCHECK(!common_program_files_path.empty());
      } else {
...
bool MaybeHasSHA256Support() {
  ...
  CR_DCHECK(os_info->version() >= VERSION_VISTA);
  return true;  // New enough to have SHA-256 support.
}

bool IsUser32AndGdi32Available() {
  static crbase::LazyInstance<LazyIsUser32AndGdi32Available>::Leaky available =
