...
#ifndef MINI_CHROMIUM_CRBASE_WIN_SHORTCUT_H_
#define MINI_CHROMIUM_CRBASE_WIN_SHORTCUT_H_
...
struct CRBASE_EXPORT ShortcutProperties {
  ...
  void set_arguments(const string16& arguments_in) {
    ...
    CR_DCHECK(arguments_in.size() < MAX_PATH);
  ...
  void set_description(const string16& description_in) {
    ...
    CR_DCHECK(description_in.size() < MAX_PATH);
...
CRBASE_EXPORT bool CreateOrUpdateShortcutLink(
...
CRBASE_EXPORT bool ResolveShortcutProperties(const FilePath& shortcut_path,
                                             uint32_t options,
...
CRBASE_EXPORT bool ResolveShortcut(const FilePath& shortcut_path,
                                   FilePath* target_path,
                                   string16* args);

// Pin to taskbar is only supported on Windows 7 and Windows 8. Returns true on
// those platforms.
CRBASE_EXPORT bool CanPinShortcutToTaskbar();
...
CRBASE_EXPORT bool PinShortcutToTaskbar(const FilePath& shortcut);
...
#endif  // MINI_CHROMIUM_CRBASE_WIN_SHORTCUT_H_