...
#include "crbase/win/open_file_name_win.h"
...
OpenFileName::OpenFileName(HWND parent_window, DWORD flags) {
  ...
  openfilename_.nMaxFile = cr_arraysize(filename_buffer_);
...
void OpenFileName::SetFilters(
  ...
  for (const auto& filter : filters) {
    filter_buffer_.append(crbase::get<0>(filter));
    filter_buffer_.push_back(0);
    filter_buffer_.append(crbase::get<1>(filter));
    filter_buffer_.push_back(0);
  }
  filter_buffer_.push_back(0);
...
void OpenFileName::SetInitialSelection(
    const crbase::FilePath& initial_directory,
  ...
  crbase::wcslcpy(filename_buffer_,
                initial_filename.value().c_str(),
                cr_arraysize(filename_buffer_));
}
...
void OpenFileName::MaybeInstallWindowPositionHookForSaveAsOnXP() {
  ...
  CR_DCHECK(!openfilename_.lpfnHook);
  openfilename_.lpfnHook = &SaveAsDialogHook;
}

crbase::FilePath OpenFileName::GetSingleResult() {
  crbase::FilePath directory;
  std::vector<crbase::FilePath> filenames;
  GetResult(&directory, &filenames);
  if (filenames.size() != 1)
    return crbase::FilePath();
  return directory.Append(filenames[0]);
}

void OpenFileName::GetResult(crbase::FilePath* directory,
                             std::vector<crbase::FilePath>* filenames) {
  CR_DCHECK(filenames->empty());
  ...
  if (openfilename_.Flags & OFN_ALLOWMULTISELECT) {
    while (*selection) {  // Empty string indicates end of list.
      filenames->push_back(crbase::FilePath(selection));
...
// static
void OpenFileName::SetResult(const crbase::FilePath& directory,
                             const std::vector<crbase::FilePath>& filenames,
                             OPENFILENAME* openfilename) {
  crbase::string16 filename_value;
  if (filenames.size() == 1) {
    filename_value = directory.Append(filenames[0]).value();
  } else {
    filename_value = directory.value();
    filename_value.push_back(0);
    for (std::vector<crbase::FilePath>::const_iterator it = filenames.begin();
         it != filenames.end();
         ++it) {
...
// static
std::vector<crbase::Tuple<crbase::string16, crbase::string16>>
OpenFileName::GetFilters(const OPENFILENAME* openfilename) {
  std::vector<crbase::Tuple<crbase::string16, crbase::string16>> filters;

  const crbase::char16* display_string = openfilename->lpstrFilter;
  if (!display_string)
    return filters;

  while (*display_string) {
    const crbase::char16* display_string_end = display_string;
    while (*display_string_end)
      ++display_string_end;
    const crbase::char16* pattern = display_string_end + 1;
    const crbase::char16* pattern_end = pattern;
    while (*pattern_end)
      ++pattern_end;
    filters.push_back(
        crbase::MakeTuple(crbase::string16(display_string, display_string_end),
                  crbase::string16(pattern, pattern_end)));
