...
#ifndef MINI_CHROMIUM_CRBASE_THREADING_SEQUENCED_WORKER_POOL_H_
#define MINI_CHROMIUM_CRBASE_THREADING_SEQUENCED_WORKER_POOL_H_
...
class CRBASE_EXPORT SequencedWorkerPool : public TaskRunner {
 public:
  ...
  bool PostWorkerTask(const crtracked_objects::Location& from_here,
                      const Closure& task);
  ...
  // (BLOCK_SHUTDOWN semantics).
  bool PostDelayedWorkerTask(const crtracked_objects::Location& from_here,
                             const Closure& task,
                             TimeDelta delay);
  ...
  bool PostNamedSequencedWorkerTask(
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(SequencedWorkerPool);
};

}  // namespace crbase

#endif  // MINI_CHROMIUM_CRBASE_THREADING_SEQUENCED_WORKER_POOL_H_