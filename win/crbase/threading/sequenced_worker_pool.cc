...
#include "crbase/atomic_sequence_num.h"
#include "crbase/callback.h"
#include "crbase/compiler_specific.h"
#include "crbase/critical_closure.h"
...
#include "crbase/thread_task_runner_handle.h"
...
struct SequencedTask : public TrackingInfo  {
  ...
  explicit SequencedTask(const crtracked_objects::Location& from_here)
...
class SequencedWorkerPoolTaskRunner : public TaskRunner {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(SequencedWorkerPoolTaskRunner);
};
...
class SequencedWorkerPoolSequencedTaskRunner : public SequencedTaskRunner {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(SequencedWorkerPoolSequencedTaskRunner);
};
...
class SequencedWorkerPool::Worker : public SimpleThread {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(Worker);
};
...
class SequencedWorkerPool::Inner {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(Inner);
};
...
LazyInstance<ThreadLocalPointer<SequencedWorkerPool::Worker>>::Leaky
    SequencedWorkerPool::Worker::lazy_tls_ptr_ = CR_LAZY_INSTANCE_INITIALIZER;
...
bool SequencedWorkerPool::Inner::PostTask(
  ...
  sequenced.task =
      ...
      crbase::MakeCriticalClosure(task) : task;
...
void SequencedWorkerPool::Inner::Shutdown(
  ...
  CR_DCHECK_GE(max_new_blocking_tasks_after_shutdown, 0);
  {
    AutoLock lock(lock_);
    ...
    while (true) {
      ...
      if (status == GET_WORK_FOUND) {
        ...
        int new_thread_id = WillRunWorkerTask(task);
        {
          ...
          crtracked_objects::TaskStopwatch stopwatch;
          stopwatch.Start();
          task.task.Run();
          stopwatch.Stop();

          crtracked_objects::ThreadData::TallyRunOnNamedThreadIfTracking(
              task, stopwatch);
...
void SequencedWorkerPool::Inner::DidRunWorkerTask(const SequencedTask& task) {
  ...
  if (task.shutdown_behavior != CONTINUE_ON_SHUTDOWN) {
    CR_DCHECK_GT(blocking_shutdown_thread_count_, 0u);
    blocking_shutdown_thread_count_--;
  }
