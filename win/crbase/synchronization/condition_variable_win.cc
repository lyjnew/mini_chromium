...
void WinVistaCondVar::TimedWait(const TimeDelta& max_time) {
  ...
  if (FALSE == sleep_condition_variable_fn(&cv_, cs, timeout)) {
    CR_DCHECK(GetLastError() != WAIT_TIMEOUT);
  }
 ...
 public:
  ...
  class Event {
   ...
   private:
    ...
    Event* next_;
    Event* prev_;
    CR_DISALLOW_COPY_AND_ASSIGN(Event);
  };
...
WinXPCondVar::~WinXPCondVar() {
  ...
  CR_DCHECK_EQ(recycling_list_size_, allocation_counter_);
...
WinXPCondVar::Event* WinXPCondVar::GetEventForWaiting() {
  ...
  if (0 == recycling_list_size_) {
    CR_DCHECK(recycling_list_.IsEmpty());
    cv_event = new Event();
    cv_event->InitListElement();
    allocation_counter_++;
    CR_DCHECK(cv_event->handle());
  } else {
...
WinXPCondVar::Event::~Event() {
  if (0 == handle_) {
    ...
    while (!IsEmpty()) {
      ...
      CR_DCHECK(cv_event->ValidateAsItem());
      delete cv_event;
    }
  }
  CR_DCHECK(IsSingleton());
  if (0 != handle_) {
...
bool WinXPCondVar::Event::IsEmpty() const {
  CR_DCHECK(ValidateAsList());
  return IsSingleton();
}

void WinXPCondVar::Event::PushBack(Event* other) {
  CR_DCHECK(ValidateAsList());
  CR_DCHECK(other->ValidateAsItem());
  CR_DCHECK(other->IsSingleton());
  ...
  prev_->next_ = other;
  prev_ = other;
  CR_DCHECK(ValidateAsDistinct(other));
}
