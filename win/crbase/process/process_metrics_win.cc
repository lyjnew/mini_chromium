...
ProcessMetrics::ProcessMetrics(ProcessHandle process)
    : process_(process),
      processor_count_(crbase::SysInfo::NumberOfProcessors()),
      last_system_time_(0) {
}
...
size_t GetSystemCommitCharge() {
  ...
  if (!InternalGetPerformanceInfo(&info, sizeof(info))) {
    CR_DLOG(ERROR) << "Failed to fetch internal performance info.";
    return 0;
  }
...
bool GetSystemMemoryInfo(SystemMemoryInfoKB* meminfo) {
  ...
  meminfo->free = static_cast<int>(mem_status.ullAvailPhys / 1024);
