...
bool GetAppOutputInternal(const StringPiece16& cl,
 ...
 crbase::win::ScopedProcessInformation proc_info(temp_process_info);
...
Process LaunchProcess(const string16& cmdline,
  ...
  if (options.as_user) {
    ...
    if (!launched) {
      CR_DPLOG(ERROR) << "Command line:" << std::endl << UTF16ToUTF8(cmdline)
                      << std::endl;
      return Process();
    }
  } else {
  ...
  crbase::win::ScopedProcessInformation process_info(temp_process_info);
