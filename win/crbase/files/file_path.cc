...
const FilePath::CharType kStringTerminator = CR_FILE_PATH_LITERAL('\0');
...
#if defined(CR_FILE_PATH_USES_DRIVE_LETTERS)
...
#endif  // defined(CR_FILE_PATH_USES_DRIVE_LETTERS)
...
#else  // CR_FILE_PATH_USES_DRIVE_LETTERS
  // Look for a separator in the first position.
  return path.length() > 0 && FilePath::IsSeparator(path[0]);
#endif  // CR_FILE_PATH_USES_DRIVE_LETTERS
}
...
StringType::size_type ExtensionSeparatorPosition(const StringType& path) {
  ...
  for (size_t i = 0; i < cr_arraysize(kCommonDoubleExtensions); ++i) {
  ...
  for (size_t i = 0; i < cr_arraysize(kCommonDoubleExtensionSuffixes); ++i) {
...
#else  // defined(CR_FILE_PATH_USES_DRIVE_LETTERS)
...
bool FilePath::ReferencesParent() const {
  ...
  for (; it != components.end(); ++it) {
    ...
    if (component.find_first_not_of(CR_FILE_PATH_LITERAL(". \n\r\t")) ==
            std::string::npos &&
...
FilePath FilePath::NormalizePathSeparatorsTo(CharType separator) const {
  ...
  CR_DCHECK_NE(kSeparators + kSeparatorsLength,
               std::find(kSeparators, kSeparators + kSeparatorsLength, 
                         separator));
