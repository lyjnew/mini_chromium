...
bool CreateTemporaryFileInDir(const FilePath& dir, FilePath* temp_file) {
  ...
  if (!GetTempFileName(dir.value().c_str(), L"", 0, temp_name)) {
    CR_DPLOG(WARNING) << "Failed to get temporary file name in "
...
int WriteFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(CreateFile(filename.value().c_str(),
                                    ...
                                    NULL));
  if (!file.IsValid()) {
    CR_DPLOG(WARNING) << "CreateFile failed for path "
  ...
  if (!result) {
    // WriteFile failed.
    CR_DPLOG(WARNING) << "writing file " << UTF16ToUTF8(filename.value())
                      << " failed";
  } else {
    // Didn't write all the bytes.
    CR_DLOG(WARNING) << "wrote" << written << " bytes to "
...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(CreateFile(filename.value().c_str(),
                                    ...
                                    NULL));
  if (!file.IsValid()) {
    CR_VPLOG(1) << "CreateFile failed for path " 
                << UTF16ToUTF8(filename.value());
    return false;
  }
  ...
  if (!result) {
    // WriteFile failed.
    CR_VPLOG(1) << "Writing file " << UTF16ToUTF8(filename.value()) 
                << " failed";
  } else {
    // Didn't write all the bytes.
    CR_VPLOG(1) << "Only wrote " << written << " out of " << size 
                << " byte(s) to " << UTF16ToUTF8(filename.value());
  }
  return false;
}
