...
void MicrosecondsToFileTime(int64_t us, FILETIME* ft) {
  CR_DCHECK_GE(us, 0LL) << "Time is less than 0, negative values are not "
...
crbase::LazyInstance<crbase::Lock>::Leaky g_high_res_lock =
...
void Time::EnableHighResolutionTimer(bool enable) {
  crbase::AutoLock lock(g_high_res_lock.Get());
...
bool Time::ActivateHighResolutionTimer(bool activating) {
  ...
  if (activating) {
    CR_DCHECK_NE(g_high_res_timer_count, max);
    ++g_high_res_timer_count;
    if (g_high_res_timer_count == 1)
      timeBeginPeriod(period);
  } else {
    CR_DCHECK_NE(g_high_res_timer_count, 0u);
    --g_high_res_timer_count;
...
Time Time::FromExploded(bool is_local, const Exploded& exploded) {
  ...
  if (!success) {
    CR_NOTREACHED() << "Unable to convert time";
    return Time(0);
  }
  return Time(FileTimeToMicroseconds(ft));
}
...
void Time::Explode(bool is_local, Exploded* exploded) const {
  ...
  if (!success) {
    CR_NOTREACHED() << "Unable to convert time, don't know why";
    ZeroMemory(exploded, sizeof(*exploded));
    return;
  }
...
TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value) {
  ...
  CR_DCHECK_GT(g_qpc_ticks_per_second, 0);
...
double ThreadTicks::TSCTicksPerSecond() {
  ...
  CR_DCHECK_GE(perf_counter_now, perf_counter_initial);
  ...
  CR_DCHECK_GE(tsc_now, tsc_initial);
