...
SharedMemoryHandle::SharedMemoryHandle(HANDLE h, crbase::ProcessId pid)
    : handle_(h), pid_(pid) {}
...
bool SharedMemoryHandle::BelongsToCurrentProcess() const {
  return pid_ == crbase::GetCurrentProcId();
}
...
crbase::ProcessId SharedMemoryHandle::GetPID() const {
  return pid_;
}
