...
#ifndef MINI_CHROMIUM_CRBASE_MEMORY_SCOPED_PTR_H_
#define MINI_CHROMIUM_CRBASE_MEMORY_SCOPED_PTR_H_
...
template <typename T> struct IsNotRefCounted {
  enum {
    value = !crbase::is_convertible<T*, crbase::subtle::RefCountedBase*>::value
        &&
        !crbase::is_convertible<T*, crbase::subtle::RefCountedThreadSafeBase*>::
            value
  };
};
...
class scoped_ptr_impl {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(scoped_ptr_impl);
};
...
class scoped_ptr {
  CR_DISALLOW_COPY_AND_ASSIGN_WITH_MOVE_FOR_BIND(scoped_ptr)

  static_assert(!std::is_array<T>::value,
                "scoped_ptr doesn't support array with size");
  static_assert(crbase::internal::IsNotRefCounted<T>::value,
 ...
 private:
  typedef crbase::internal::scoped_ptr_impl<element_type, deleter_type>
      scoped_ptr::*Testable;
 ...
 public:
  ...
  element_type* release() CR_WARN_UNUSED_RESULT {
 ...
 private:
  ...
  crbase::internal::scoped_ptr_impl<element_type, deleter_type> impl_;
...
// Free functions
template <class T, class D>
void swap(crbase::scoped_ptr<T, D>& p1, crbase::scoped_ptr<T, D>& p2) {
  p1.swap(p2);
}

template <class T1, class D1, class T2, class D2>
bool operator==(const crbase::scoped_ptr<T1, D1>& p1,
                const crbase::scoped_ptr<T2, D2>& p2) {
  return p1.get() == p2.get();
}
template <class T, class D>
bool operator==(const crbase::scoped_ptr<T, D>& p, std::nullptr_t) {
  return p.get() == nullptr;
}
template <class T, class D>
bool operator==(std::nullptr_t, const crbase::scoped_ptr<T, D>& p) {
  return p.get() == nullptr;
}
...
template <typename T>
crbase::scoped_ptr<T> make_crbase_scoped_ptr(T* ptr) {
  return crbase::scoped_ptr<T>(ptr);
}

template <typename T>
std::ostream& operator<<(std::ostream& out, const crbase::scoped_ptr<T>& p) {
  return out << p.get();
}

#endif  // MINI_CHROMIUM_CRBASE_MEMORY_SCOPED_PTR_H_