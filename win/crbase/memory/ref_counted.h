...
#endif
#include "crbase/threading/thread_collision_warner.h"
...
class CRBASE_EXPORT RefCountedBase {
 ...
 protected:
  ...
  ~RefCountedBase() {
  #ifndef NDEBUG
    CR_DCHECK(in_dtor_) << "RefCounted object deleted without calling Release()";
  #endif
  }
 ...
 private:
  ...
  CR_DFAKE_MUTEX(add_release_);

  CR_DISALLOW_COPY_AND_ASSIGN(RefCountedBase);
};
...
class CRBASE_EXPORT RefCountedThreadSafeBase {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(RefCountedThreadSafeBase);
};
...
class RefCounted : public subtle::RefCountedBase {
 ...
 private:
  CR_DISALLOW_COPY_AND_ASSIGN(RefCounted<T>);
};
...
class RefCountedThreadSafe : public subtle::RefCountedThreadSafeBase {
 ...
 private:
  ...
  CR_DISALLOW_COPY_AND_ASSIGN(RefCountedThreadSafe);
};
...
template <typename T>
crbase::scoped_refptr<T> make_crbase_scoped_refptr(T* t) {
  return crbase::scoped_refptr<T>(t);
}

// Temporary operator overloads to facilitate the transition. See
// https://crbug.com/110610.
template <typename T, typename U>
bool operator==(const crbase::scoped_refptr<T>& lhs, const U* rhs) {
  return lhs.get() == rhs;
}

template <typename T, typename U>
bool operator==(const T* lhs, const crbase::scoped_refptr<U>& rhs) {
  return lhs == rhs.get();
}
...
bool operator!=(const T* lhs, 
                const crbase::scoped_refptr<U>& rhs) {
  return !operator==(lhs, rhs);
}
