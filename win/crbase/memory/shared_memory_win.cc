...
#include "crbase/memory/shared_memory.h"
...
bool IsSectionSafeToMap(HANDLE handle) {
  ...
  if (!nt_query_section_func) {
    ...
    CR_DCHECK(nt_query_section_func);
  }
...
SharedMemory::SharedMemory(const SharedMemoryHandle& handle, bool read_only)
    : external_section_(true),
      ...
      requested_size_(0) {
  CR_DCHECK(!handle.IsValid() || handle.BelongsToCurrentProcess());
}
...
SharedMemoryHandle SharedMemory::DuplicateHandle(
  ...
  CR_DCHECK(handle.BelongsToCurrentProcess());
  HANDLE duped_handle;
...
bool SharedMemory::Create(const SharedMemoryCreateOptions& options) {
  ...
  CR_DCHECK(!options.executable);
  CR_DCHECK(!mapped_file_);
  if (options.size == 0)
    return false;
...
bool SharedMemory::MapAt(off_t offset, size_t bytes) {
  ...
  if (memory_ != NULL) {
    CR_DCHECK_EQ(0U, reinterpret_cast<uintptr_t>(memory_) &
...
bool SharedMemory::ShareToProcessCommon(ProcessHandle process,
  ...
  *new_handle = SharedMemoryHandle();
  ...
  if (process == GetCurrentProcess() && close_self) {
    *new_handle = SharedMemoryHandle(mapped_file, crbase::GetCurrentProcId());
    return true;
  }
  ...
  *new_handle = SharedMemoryHandle(result, crbase::GetProcId(process));
  return true;
}
...
SharedMemoryHandle SharedMemory::handle() const {
  return SharedMemoryHandle(mapped_file_, crbase::GetCurrentProcId());
}
