...
// 际上产生了规范里未定义的行为. 段落讲到:如果一个对象在内存拥有一个类型并且程序以另外一
// 个不同的类型访问它, 这结果对于大多数的值来说是未定义的行为.
// 
// 这对于任何转换语法来说是真的, 不管是*(int*)&f还是*reinterpret_cast<int*>(&f),
// 尤其是在积分左值和浮点左值转换的时候.
// 
// 规范的目的是去允许优化编译器去假设具有不用类型的表达式引用不同的内存, 编译器都知道
// 去利用这一点, 因此一个不合格的程序会悄悄粗鲁的产生错误输出
//
// 问题不在于reinterpret_cast的使用, 而在于类型双语关:
...
//
// 警告: 如果模板参数目标(Dest)或目标(Source)是一个non-POD类型, memcpy的结果可能会
// 让你惊讶
// POD: 普通旧版数据, 比如int, float等C基础类型, C++里的结构体、类就是非普通旧版数据
