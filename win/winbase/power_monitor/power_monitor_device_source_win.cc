...
const wchar_t kWindowClassName[] = L"WinBase_PowerMessageWindow";
...
bool PowerMonitorDeviceSource::IsOnBatteryPowerImpl() {
  ...
  if (!GetSystemPowerStatus(&status)) {
    WINBASE_DPLOG(ERROR) << "GetSystemPowerStatus failed";
    return false;
  }
...
PowerMonitorDeviceSource::PowerMessageWindow::PowerMessageWindow()
  ...
  winbase::win::InitializeWindowClass(
      kWindowClassName,
      &winbase::win::WrappedWindowProc<
          PowerMonitorDeviceSource::PowerMessageWindow::WndProcThunk>,
      0, 0, 0, NULL, NULL, NULL, NULL, NULL,
      &window_class);
  instance_ = window_class.hInstance;
  ATOM clazz = RegisterClassEx(&window_class);
  WINBASE_DCHECK(clazz);
