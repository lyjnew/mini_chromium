...
class BaseTimerTaskInternal {
 public:
  ...
  BaseTimerTaskInternal(const BaseTimerTaskInternal&) = delete;
  BaseTimerTaskInternal& operator=(const BaseTimerTaskInternal&) = delete;
...
TimerBase::~TimerBase() {
  WINBASE_DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  AbandonAndStop();
}
...
void TimerBase::Reset() {
  WINBASE_DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  WINBASE_DCHECK(!user_task_.is_null());
...
void TimerBase::PostNewScheduledTask(TimeDelta delay) {
  ...
  // RunScheduledTask(): https://crbug.com/587199.
  // DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  WINBASE_DCHECK(!scheduled_task_);
  is_running_ = true;
  ...
  if (delay > TimeDelta::FromMicroseconds(0)) {
    ...
    GetTaskRunner()->PostDelayedTask(
        posted_from_,
        winbase::BindOnce(&BaseTimerTaskInternal::Run,
                          winbase::Owned(scheduled_task_)),
        delay);
...
void TimerBase::RunScheduledTask() {
  ...
  winbase::Closure task = user_task_;
...
void OneShotTimer::FireNow() {
  WINBASE_DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  WINBASE_DCHECK(!task_runner_) << "FireNow() is incompatible with SetTaskRunner()";
  WINBASE_DCHECK(IsRunning());

  OnceClosure task = user_task();
  Stop();
  WINBASE_DCHECK(!user_task());
  std::move(task).Run();
}
