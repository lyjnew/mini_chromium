...
#ifndef WINLIB_WINBASE_SYNCHRONIZATION_SPIN_WAIT_H_
#define WINLIB_WINBASE_SYNCHRONIZATION_SPIN_WAIT_H_
...
//
//   WINBASE_SPIN_FOR_1_SECOND_OR_UNTIL_TRUE(0 == f(x));
//
...
#define WINBASE_SPIN_FOR_1_SECOND_OR_UNTIL_TRUE(expression) \
  WINBASE_SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(                 \
      winbase::TimeDelta::FromSeconds(1), (expression))

#define WINBASE_SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(delta, expression)            \
  do {                                                                         \
  winbase::TimeTicks start = winbase::TimeTicks::Now();                        \
  const winbase::TimeDelta kTimeout = delta;                                   \
    while (!(expression)) {                                                    \
      if (kTimeout < winbase::TimeTicks::Now() - start) {                      \
      WINBASE_EXPECT_LE((winbase::TimeTicks::Now() - start).InMilliseconds(),  \
                        kTimeout.InMilliseconds()) << "Timed out";             \
        break;                                                                 \
      }                                                                        \
      winbase::PlatformThread::Sleep(winbase::TimeDelta::FromMilliseconds(50));\
    }                                                                          \
  } while (0)

#endif  // WINLIB_WINBASE_SYNCHRONIZATION_SPIN_WAIT_H_