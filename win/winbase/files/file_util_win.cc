  ...
  for (FilePath current = traversal.Next(); !current.empty();
    ...
    if (info.IsDirectory()) {
      if (recursive) {
        ...
        WINBASE_DCHECK_NE(static_cast<LONG>(this_result), ERROR_FILE_NOT_FOUND);
        WINBASE_DCHECK_NE(static_cast<LONG>(this_result), ERROR_PATH_NOT_FOUND);
...
void AppendModeCharacter(winbase::char16 mode_char, winbase::string16* mode) {
  size_t comma_pos = mode->find(L',');
  mode->insert(comma_pos == winbase::string16::npos ? 
                   mode->length() : comma_pos, 
               1, mode_char);
}
...
bool DoCopyDirectory(const FilePath& from_path,
  ...
  if (!PathExists(from_path)) {
    WINBASE_DLOG(ERROR) << "CopyDirectory() couldn't stat source directory: "
                        << from_path.value().c_str();
    return false;
  }
  // TODO(maruel): This is not necessary anymore.
  WINBASE_DCHECK(recursive || DirectoryExists(from_path));
  ...
  while (success && !current.empty()) {
    ...
    if (from_is_dir) {
      if (!DirectoryExists(target_path) &&
        ...
        WINBASE_DLOG(ERROR) << "CopyDirectory() couldn't create directory: "
                            << target_path.value().c_str();
        success = false;
      }
    } else if (!DoCopyFile(current, target_path, fail_if_exists)) {
      WINBASE_DLOG(ERROR) << "CopyDirectory() couldn't create file: "
                          << target_path.value().c_str();
      success = false;
    }
...
DWORD DoDeleteFile(const FilePath& path, bool recursive) {
  ...
  if (path.BaseName().value().find_first_of(L"*?") !=
    ...
    WINBASE_DCHECK_NE(static_cast<LONG>(error_code), ERROR_FILE_NOT_FOUND);
    WINBASE_DCHECK_NE(static_cast<LONG>(error_code), ERROR_PATH_NOT_FOUND);
    return error_code;
  }
...
bool CreateTemporaryFileInDir(const FilePath& dir, FilePath* temp_file) {
  ...
  if (!create_file_success) {
    WINBASE_DPLOG(WARNING) << "Failed to get temporary file name in "
...
bool CreateDirectoryAndGetError(const FilePath& full_path,
  ...
  if (fileattr != INVALID_FILE_ATTRIBUTES) {
    if ((fileattr & FILE_ATTRIBUTE_DIRECTORY) != 0) {
      WINBASE_DVLOG(1) << "CreateDirectory(" << full_path_str << "), "
                       << "directory already exists.";
      return true;
    }
    WINBASE_DLOG(WARNING) << "CreateDirectory(" << full_path_str << "), "
  ...
  if (!CreateDirectoryAndGetError(parent_path, error)) {
    WINBASE_DLOG(WARNING) << "Failed to create one of the parent directories.";
    if (error) {
      WINBASE_DCHECK(*error != File::FILE_OK);
    }
    return false;
  }
  ...
  if (!::CreateDirectory(full_path_str, NULL)) {
    ...
    if (error_code == ERROR_ALREADY_EXISTS && DirectoryExists(full_path)) {
      ...
      WINBASE_DLOG(WARNING) << "Failed to create directory " << full_path_str
...
bool DevicePathToDriveLetterPath(const FilePath& nt_device_path,
  ...
  if (!::GetLogicalDriveStrings(kDriveMappingSize - 1, drive_mapping)) {
    WINBASE_DLOG(ERROR) << "Failed to get drive mapping.";
    return false;
  }
...
int WriteFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(CreateFile(filename.value().c_str(), GENERIC_WRITE, 0,
                                    ...
                                    NULL));
  if (!file.IsValid()) {
    WINBASE_DPLOG(WARNING) << "CreateFile failed for path "
  ...
  if (!result) {
    // WriteFile failed.
    WINBASE_DPLOG(WARNING) << "writing file " << UTF16ToUTF8(filename.value())
                           << " failed";
  } else {
    // Didn't write all the bytes.
    WINBASE_DLOG(WARNING) << "wrote" << written << " bytes to "
                          << UTF16ToUTF8(filename.value()) 
                          << " expected " << size;
  }
  return -1;
...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(CreateFile(filename.value().c_str(),
                                    ...
                                    NULL));
  if (!file.IsValid()) {
    WINBASE_VPLOG(1) << "CreateFile failed for path "
                     << UTF16ToUTF8(filename.value());
    return false;
  }
  ...
  if (!result) {
    // WriteFile failed.
    WINBASE_VPLOG(1) << "Writing file " << UTF16ToUTF8(filename.value())
                     << " failed";
  } else {
    // Didn't write all the bytes.
    WINBASE_VPLOG(1) << "Only wrote " << written << " out of " << size
                     << " byte(s) to "
                     << UTF16ToUTF8(filename.value());
  }
  return false;
