...
void File::Close() {
  ...
  WINBASE_SCOPED_FILE_TRACE("Close");
  file_.Close();
}
...
int64_t File::Seek(Whence whence, int64_t offset) {
  ...
  WINBASE_SCOPED_FILE_TRACE_WITH_SIZE("Seek", offset);
...
int File::Read(int64_t offset, char* data, int size) {
  AssertBlockingAllowed();
  WINBASE_DCHECK(IsValid());
  WINBASE_DCHECK(!async_);
  if (size < 0)
    return -1;

  WINBASE_SCOPED_FILE_TRACE_WITH_SIZE("Read", size);
...
int File::Write(int64_t offset, const char* data, int size) {
  ...
  WINBASE_SCOPED_FILE_TRACE_WITH_SIZE("Write", size);
...
int File::WriteAtCurrentPos(const char* data, int size) {
  ...
  WINBASE_SCOPED_FILE_TRACE_WITH_SIZE("WriteAtCurrentPos", size);
...
int64_t File::GetLength() {
  ...
  WINBASE_SCOPED_FILE_TRACE("GetLength");
...
bool File::SetLength(int64_t length) {
  ...
  WINBASE_SCOPED_FILE_TRACE_WITH_SIZE("SetLength", length);
...
bool File::SetTimes(Time last_access_time, Time last_modified_time) {
  ...
  WINBASE_SCOPED_FILE_TRACE("SetTimes");
...
bool File::GetInfo(Info* info) {
  ...
  WINBASE_SCOPED_FILE_TRACE("GetInfo");
...
File::Error File::Lock() {
  ...
  WINBASE_SCOPED_FILE_TRACE("Lock");
...
File::Error File::Unlock() {
  ...
  WINBASE_SCOPED_FILE_TRACE("Unlock");
...
File File::Duplicate() const {
  ...
  WINBASE_SCOPED_FILE_TRACE("Duplicate");
...
File::Error File::OSErrorToFileError(DWORD last_error) {
  switch (last_error) {
    ...
    default:
      ...
      WINBASE_DCHECK_NE(static_cast<DWORD>(ERROR_SUCCESS), last_error);
...
void File::DoInitialize(const FilePath& path, uint32_t flags) {
  ...
  if (flags & FLAG_CREATE_ALWAYS) {
    WINBASE_DCHECK(!disposition);
    WINBASE_DCHECK(flags & FLAG_WRITE);
...
bool File::Flush() {
  AssertBlockingAllowed();
  WINBASE_DCHECK(IsValid());
  WINBASE_SCOPED_FILE_TRACE("Flush");
