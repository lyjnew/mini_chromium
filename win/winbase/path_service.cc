...
bool PathService::Get(int key, FilePath* result) {
  ...
  WINBASE_DCHECK(path_data);
  WINBASE_DCHECK(result);
  WINBASE_DCHECK_GE(key, DIR_CURRENT);
  ...
  while (provider) {
    if (provider->func(key, &path))
      break;
    WINBASE_DCHECK(path.empty()) << "provider should not have modified path";
...
bool PathService::OverrideAndCreateIfNeeded(int key,
  ...
  PathData* path_data = GetPathData();
  WINBASE_DCHECK(path_data);
  WINBASE_DCHECK_GT(key, DIR_CURRENT) << "invalid path key";
  ...
  WINBASE_DCHECK(file_path.IsAbsolute());
...
void PathService::RegisterProvider(ProviderFunc func,
  ...
  PathData* path_data = GetPathData();
  WINBASE_DCHECK(path_data);
  WINBASE_DCHECK_GT(key_end, key_start);
  ...
  while (iter) {
    WINBASE_DCHECK(key_start >= iter->key_end || key_end <= iter->key_start)
