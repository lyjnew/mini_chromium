...
#ifndef WINLIB_WINBASE_RAND_UTIL_H_
#define WINLIB_WINBASE_RAND_UTIL_H_
...
WINBASE_EXPORT uint64_t RandUint64();

// Returns a random number between min and max (inclusive). Thread-safe.
WINBASE_EXPORT int RandInt(int min, int max);

// Returns a random number in range [0, range).  Thread-safe.
WINBASE_EXPORT uint64_t RandGenerator(uint64_t range);

// Returns a random double in range [0, 1). Thread-safe.
WINBASE_EXPORT double RandDouble();

// Given input |bits|, convert with maximum precision to a double in
// the range [0, 1). Thread-safe.
WINBASE_EXPORT double BitsToOpenEndedUnitInterval(uint64_t bits);
...
WINBASE_EXPORT void RandBytes(void* output, size_t output_length);
...
// random number source, code outside of base/ that relies on this should use
// crypto::RandBytes instead to ensure the requirement is easily discoverable.
WINBASE_EXPORT std::string RandBytesAsString(size_t length);
...
#endif  // WINLIB_WINBASE_RAND_UTIL_H_