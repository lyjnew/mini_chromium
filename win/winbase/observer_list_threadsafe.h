...
#ifndef WINLIB_WINBASE_OBSERVER_LIST_THREADSAFE_H_
#define WINLIB_WINBASE_OBSERVER_LIST_THREADSAFE_H_
...
class WINBASE_EXPORT ObserverListThreadSafeBase
...
class ObserverListThreadSafe : public internal::ObserverListThreadSafeBase {
 public:
  ...
  void AddObserver(ObserverType* observer) {
    ...
    WINBASE_DCHECK(!ContainsKey(observers_, observer));
  ...
  void NotifyWrapper(ObserverType* observer,
                     const NotificationData& notification) {
    {
      ...
      if (it == observers_.end())
        return;
      WINBASE_DCHECK(it->second->RunsTasksInCurrentSequence());
    }
...
#endif  // WINLIB_WINBASE_OBSERVER_LIST_THREADSAFE_H_