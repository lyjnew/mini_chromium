  ...
  std::unique_ptr<FileVersionInfoWin> file_version_info(
      static_cast<FileVersionInfoWin*>(
          winbase::FileVersionInfoWin::CreateFileVersionInfo(
              winbase::FilePath(WINBASE_FILE_PATH_LITERAL("kernel32.dll")))));
...
#pragma warning(pop)
  ...
  service_pack_str_ = winbase::WideToUTF8(version_info.szCSDVersion);

  SYSTEM_INFO system_info = {};
  ::GetNativeSystemInfo(&system_info);
  switch (system_info.wProcessorArchitecture) {
    case PROCESSOR_ARCHITECTURE_INTEL:
      architecture_ = WindowsArchitecture::X86_ARCHITECTURE;
      break;
    case PROCESSOR_ARCHITECTURE_AMD64:
      architecture_ = WindowsArchitecture::X64_ARCHITECTURE;
      break;
    case PROCESSOR_ARCHITECTURE_IA64:
      architecture_ = WindowsArchitecture::IA64_ARCHITECTURE;
      break;
  }
...
std::string OSInfo::processor_model_name() {
  if (processor_model_name_.empty()) {
    ...
    winbase::win::RegKey key(HKEY_LOCAL_MACHINE, kProcessorNameString, KEY_READ);
    string16 value;
