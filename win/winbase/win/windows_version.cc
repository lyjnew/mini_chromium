...
OSInfo::WindowsArchitecture OSInfo::GetArchitecture() {
  switch (GetSystemInfoStorage().wProcessorArchitecture) {
    case PROCESSOR_ARCHITECTURE_INTEL:
      return WindowsArchitecture::X86_ARCHITECTURE;
    case PROCESSOR_ARCHITECTURE_AMD64:
      return WindowsArchitecture::X64_ARCHITECTURE;
    case PROCESSOR_ARCHITECTURE_IA64:
      return WindowsArchitecture::IA64_ARCHITECTURE;
    case PROCESSOR_ARCHITECTURE_ARM64:
      return WindowsArchitecture::ARM64_ARCHITECTURE;
    default:
      return WindowsArchitecture::OTHER_ARCHITECTURE;
  }
}
...
Version OSInfo::Kernel32Version() const {
  ...
  ///winbase::ScopedAllowBlocking allow_blocking;
...
OSInfo::VersionNumber OSInfo::Kernel32VersionNumber() const {
  WINBASE_DCHECK_EQ(Kernel32BaseVersion().components().size(), 4u);
...
winbase::Version OSInfo::Kernel32BaseVersion() const {
  static const NoDestructor<winbase::Version> version([] {
    std::unique_ptr<FileVersionInfoWin> file_version_info(
        static_cast<FileVersionInfoWin*>(
            FileVersionInfoWin::CreateFileVersionInfo(
                FilePath(WINBASE_FILE_PATH_LITERAL("kernel32.dll")))));
    if (!file_version_info) {
      ...
      file_version_info.reset(static_cast<FileVersionInfoWin*>(
          FileVersionInfoWin::CreateFileVersionInfo(
              FilePath(WINBASE_FILE_PATH_LITERAL("kernelbase.dll")))));
    }
    WINBASE_CHECK(file_version_info);
    return file_version_info->GetVersion();
  }());
  return *version;
}
...
Version OSInfo::MajorMinorBuildToVersion(uint32_t major,
  ...
  if (major > 6) {
    ...
    WINBASE_NOTREACHED() << major << "." << minor << "." << build;
