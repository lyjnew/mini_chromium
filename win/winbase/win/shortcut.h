...
#ifndef WINLIB_WINBASE_WIN_SHORTCUT_H_
#define WINLIB_WINBASE_WIN_SHORTCUT_H_
...
struct WINBASE_EXPORT ShortcutProperties {
  ...
  void set_arguments(const string16& arguments_in) {
    ...
    WINBASE_DCHECK(arguments_in.size() < MAX_PATH);
  ...
  void set_description(const string16& description_in) {
    ...
    WINBASE_DCHECK(description_in.size() < MAX_PATH);
...
WINBASE_EXPORT bool CreateOrUpdateShortcutLink(
...
WINBASE_EXPORT bool ResolveShortcutProperties(const FilePath& shortcut_path,
                                              uint32_t options,
...
WINBASE_EXPORT bool ResolveShortcut(const FilePath& shortcut_path,
                                    FilePath* target_path,
                                    string16* args);

// Pin to taskbar is only supported on Windows 7 and Windows 8. Returns true on
// those platforms.
WINBASE_EXPORT bool CanPinShortcutToTaskbar();
...
WINBASE_EXPORT bool PinShortcutToTaskbar(const FilePath& shortcut);
...
WINBASE_EXPORT bool UnpinShortcutFromTaskbar(const FilePath& shortcut);

}  // namespace win
}  // namespace winbase

#endif  // WINLIB_WINBASE_WIN_SHORTCUT_H_