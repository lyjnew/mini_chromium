...
#include "winbase\win\win_util.h"
...
bool SetPropVariantValueForPropertyStore(
  ...
  // PID definitions.
  WINBASE_DPLOG(ERROR) << "Failed to set property with GUID " << guidString
                       << " PID "
                       << property_key.pid;
#endif
  return false;
...
bool SetProcessDpiAwarenessWrapper(PROCESS_DPI_AWARENESS value) {
  ...
  if (set_process_dpi_awareness_func) {
    ...
    if (SUCCEEDED(hr))
      return true;
    WINBASE_DLOG_IF(ERROR, hr == E_ACCESSDENIED)
  ...
  WINBASE_DCHECK_LT(GetVersion(), OSVersion::WIN8_1)
...
bool IsWindows10TabletMode(HWND hwnd) {
  ...
  HRESULT hr = winbase::win::RoGetActivationFactory(
...
bool SetAppIdForPropertyStore(IPropertyStore* property_store,
  ...
  WINBASE_DCHECK(lstrlen(app_id) < 64 && wcschr(app_id, L' ') == NULL);
...
bool IsDeviceRegisteredWithManagement() {
  static bool is_device_registered_with_management = []() {
    ScopedNativeLibrary library(
        FilePath(WINBASE_FILE_PATH_LITERAL("MDMRegistration.dll")));
    if (!library.is_valid())
      return false;
...
bool GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot) {
  WINBASE_DCHECK(snapshot);
  WINBASE_DCHECK_EQ(0u, snapshot->size());
  snapshot->resize(128);
  ...
  do {
    ...
    // small.
    WINBASE_DCHECK_GE(std::numeric_limits<DWORD>::max(),
                      snapshot->size() * sizeof(HMODULE));
    if (!::EnumProcessModules(
            process, &(*snapshot)[0],
            static_cast<DWORD>(snapshot->size() * sizeof(HMODULE)),
            &bytes_required)) {
      WINBASE_DPLOG(ERROR) << "::EnumProcessModules failed.";
      return false;
    }
    WINBASE_DCHECK_EQ(0u, bytes_required % sizeof(HMODULE));
    ...
    if (num_modules <= snapshot->size()) {
      ...
      WINBASE_DLOG(ERROR) << "Can't determine the module list size.";
      return false;
    } else {
  ...
  WINBASE_DLOG(ERROR) << "Failed to enumerate modules.";
  return false;
}
