...
bool CheckAndDuplicateHandle(HANDLE source, ScopedHandle* target) {
  ...
  if (!::DuplicateHandle(::GetCurrentProcess(), source,
    ...
    WINBASE_DPLOG(ERROR) << "Failed to duplicate a handle " << last_error;
...
bool ScopedProcessInformation::DuplicateFrom(
  ...
  WINBASE_DCHECK(!IsValid()) << "target ScopedProcessInformation must be NULL";
  WINBASE_DCHECK(other.IsValid())
      << "source ScopedProcessInformation must be valid";
