...
///#include "winbase\win\patch_util.h"
...
class HookManager {
 public:
  ...
  void UnregisterHook() {
    AutoLock auto_lock(lock_);
    WINBASE_DCHECK_NE(0U, init_count_);
    if (init_count_ == 1)
      RevertHook();
 ...
 private:
  ...
  void WriteHook() {
    ...
    if (format == HotpatchPlaceholderFormat::UNKNOWN) {
      WINBASE_NOTREACHED() << "Unrecognized hotpatch function format: "
    ...
    WINBASE_DCHECK_EQ(hotpatch_placeholder_format_,
