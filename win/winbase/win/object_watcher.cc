...
bool ObjectWatcher::StopWatching() {
  ...
  if (!UnregisterWaitEx(wait_object_, INVALID_HANDLE_VALUE)) {
    WINBASE_DPLOG(FATAL) << "UnregisterWaitEx failed";
    return false;
  }
...
void CALLBACK ObjectWatcher::DoneWaiting(void* param, BOOLEAN timed_out) {
  ...
  that->task_runner_->PostTask(WINBASE_FROM_HERE, that->callback_);
  if (that->run_once_)
...
bool ObjectWatcher::StartWatchingInternal(HANDLE object, Delegate* delegate,
  ...
  WINBASE_DCHECK(!wait_object_) << "Already watching an object";
  ...
  if (!RegisterWaitForSingleObject(&wait_object_, object, DoneWaiting,
    ...
    WINBASE_DPLOG(FATAL) << "RegisterWaitForSingleObject failed";
    Reset();
    return false;
  }
