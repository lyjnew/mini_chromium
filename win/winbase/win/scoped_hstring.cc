...
HRESULT WindowsCreateString(const winbase::char16* src,
                            uint32_t len,
                            HSTRING* out_hstr) {
...
const winbase::char16* WindowsGetStringRawBuffer(HSTRING hstr,
                                                 uint32_t* out_len) {
...
void ScopedHStringTraits::Free(HSTRING hstr) {
  winbase::WindowsDeleteString(hstr);
}
...
StringPiece16 ScopedHString::Get() const {
  UINT32 length = 0;
  const wchar_t* buffer = winbase::WindowsGetStringRawBuffer(get(), &length);
