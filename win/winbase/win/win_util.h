...
#ifndef WINLIB_WINBASE_WIN_WIN_UTIL_H_
#define WINLIB_WINBASE_WIN_WIN_UTIL_H_
...
// |user_sid| on failure.
WINBASE_EXPORT bool GetUserSidString(std::wstring* user_sid);
...
WINBASE_EXPORT bool UserAccountControlIsEnabled();

// Sets the boolean value for a given key in given IPropertyStore.
WINBASE_EXPORT bool SetBooleanValueForPropertyStore(
    IPropertyStore* property_store,
    const PROPERTYKEY& property_key,
    bool property_bool_value);

// Sets the string value for a given key in given IPropertyStore.
WINBASE_EXPORT bool SetStringValueForPropertyStore(
    IPropertyStore* property_store,
    const PROPERTYKEY& property_key,
    const wchar_t* property_string_value);

// Sets the CLSID value for a given key in a given IPropertyStore.
WINBASE_EXPORT bool SetClsidForPropertyStore(IPropertyStore* property_store,
                                             const PROPERTYKEY& property_key,
...
// Win7.
WINBASE_EXPORT bool SetAppIdForPropertyStore(IPropertyStore* property_store,
                                             const wchar_t* app_id);

// Adds the specified |command| using the specified |name| to the AutoRun key.
// |root_key| could be HKCU or HKLM or the root of any user hive.
WINBASE_EXPORT bool AddCommandToAutoRun(HKEY root_key,
                                        const string16& name,
                                        const string16& command);
// Removes the command specified by |name| from the AutoRun key. |root_key|
// could be HKCU or HKLM or the root of any user hive.
WINBASE_EXPORT bool RemoveCommandFromAutoRun(HKEY root_key,
                                             const string16& name);
...
WINBASE_EXPORT bool ReadCommandFromAutoRun(HKEY root_key,
                                           const string16& name,
                                           string16* command);
...
WINBASE_EXPORT void SetShouldCrashOnProcessDetach(bool crash);
WINBASE_EXPORT bool ShouldCrashOnProcessDetach();

// Adjusts the abort behavior so that crash reports can be generated when the
// process is aborted.
WINBASE_EXPORT void SetAbortBehaviorForCrashReporting();

// Checks whether the supplied |hwnd| is in Windows 10 tablet mode. Will return
// false on versions below 10.
WINBASE_EXPORT bool IsWindows10TabletMode(HWND hwnd);
...
WINBASE_EXPORT bool IsTabletDevice(std::string* reason, HWND hwnd);
...
// This function optionally sets the |reason| parameter to determine as to why
// or why not a device was deemed to be a tablet.
WINBASE_EXPORT bool IsDeviceUsedAsATablet(std::string* reason);
...
WINBASE_EXPORT bool IsKeyboardPresentOnSlate(std::string* reason, HWND hwnd);

// Get the size of a struct up to and including the specified member.
// This is necessary to set compatible struct sizes for different versions
// of certain Windows APIs (e.g. SystemParametersInfo).
#define WINBASE_SIZEOF_STRUCT_WITH_SPECIFIED_LAST_MEMBER(struct_name, member) \
    offsetof(struct_name, member) + \
    (sizeof static_cast<struct_name*>(NULL)->member)

// Returns true if the machine is enrolled to a domain.
WINBASE_EXPORT bool IsEnrolledToDomain();

// Returns true if the machine is being managed by an MDM system.
WINBASE_EXPORT bool IsDeviceRegisteredWithManagement();
...
WINBASE_EXPORT bool IsUser32AndGdi32Available();
...
WINBASE_EXPORT bool GetLoadedModulesSnapshot(
    HANDLE process,
...
WINBASE_EXPORT void EnableFlicks(HWND hwnd);
WINBASE_EXPORT void DisableFlicks(HWND hwnd);

// Returns true if the process is per monitor DPI aware.
WINBASE_EXPORT bool IsProcessPerMonitorDpiAware();

// Enable high-DPI support for the current process.
WINBASE_EXPORT void EnableHighDPISupport();

}  // namespace win
}  // namespace winbase

#endif  // WINLIB_WINBASE_WIN_WIN_UTIL_H_