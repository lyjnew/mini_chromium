...
TimeTicks CalculateDelayedRuntime(TimeDelta delay) {
  ...
  if (delay > TimeDelta())
    ...
    WINBASE_DCHECK_EQ(delay.InMilliseconds(), 0) 
...
bool IncomingTaskQueue::AddToIncomingQueue(const Location& from_here,
  ...
  // for details.
  WINBASE_CHECK(task);
  WINBASE_DLOG_IF(WARNING, delay > kTaskDelayWarningThreshold)
...
const PendingTask& IncomingTaskQueue::TriageQueue::Peek() {
  WINBASE_DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
...
void IncomingTaskQueue::TriageQueue::Clear() {
  ...
  WINBASE_DCHECK(outer_->accept_new_tasks_);
...
PendingTask IncomingTaskQueue::DelayedQueue::Pop() {
  ...
  if (delayed_task.is_high_res)
    --pending_high_res_tasks_;
  WINBASE_DCHECK_GE(pending_high_res_tasks_, 0);
...
void IncomingTaskQueue::ReloadWorkQueue(TaskQueue* work_queue) {
  ...
  WINBASE_DCHECK(work_queue->empty());
