...
void MessagePumpForUI::WaitForWork() {
  ...
  while ((delay = GetCurrentDelay()) != 0) {
    ...
    winbase::debug::Alias(&delay);
    winbase::debug::Alias(&wait_flags);
    ...
    WINBASE_DCHECK_NE(WAIT_FAILED, result) << GetLastError();
  }
}
...
void MessagePumpForUI::RescheduleTimer() {
  ...
  WINBASE_DCHECK_GE(delay_msec, 0);
  if (delay_msec == 0) {
    ScheduleWork();
  } else {
    ...
    // Tell the optimizer to retain these values to simplify analyzing hangs.
    winbase::debug::Alias(&delay_msec);
...
bool MessagePumpForUI::ProcessPumpReplacementMessage() {
  ...
  WINBASE_DCHECK(!have_message || kMsgHaveWork != msg.message ||
                 msg.hwnd != message_window_.hwnd());

  // Since we discarded a kMsgHaveWork message, we must update the flag.
  int old_work_state_ = InterlockedExchange(&work_state_, READY);
  WINBASE_DCHECK_EQ(HAVE_WORK, old_work_state_);
...
void MessagePumpForIO::WaitForWork() {
  ...
  // recursion problems.
  WINBASE_DCHECK_EQ(1, state_->run_depth) << "Cannot nest an IO message loop!";
...
bool MessagePumpForIO::ProcessInternalIOItem(const IOItem& item) {
  if (reinterpret_cast<void*>(this) == reinterpret_cast<void*>(item.context) &&
    ...
    WINBASE_DCHECK(!item.bytes_transfered);
    InterlockedExchange(&work_state_, READY);
    return true;
  }
  return false;
...
bool MessagePumpForIO::MatchCompletedIOItem(IOHandler* filter, IOItem* item) {
  WINBASE_DCHECK(!completed_io_.empty());
