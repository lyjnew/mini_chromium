...
#include "winbase\debug\task_annotator.h"
#include "winbase\logging.h"
#include "winbase\memory\ptr_util.h"
#include "winbase\message_loop\message_pump_default.h"
...
class MessageLoop::Controller : public internal::IncomingTaskQueue::Observer {
 ...
 private:
  ...
  // members below.
  winbase::Lock message_loop_lock_;
...
void MessageLoop::Controller::StartScheduling() {
  ...
  WINBASE_DCHECK(message_loop_);
  WINBASE_DCHECK(!is_ready_for_scheduling_);
...
void MessageLoop::BindToCurrentThread() {
  ...
  WINBASE_DCHECK(!MessageLoopCurrent::IsSet())
...
void MessageLoop::SetTaskRunner(
  ...
  WINBASE_DCHECK(task_runner);
  WINBASE_DCHECK(task_runner->BelongsToCurrentThread());
...
void MessageLoop::Run(bool application_tasks_allowed) {
  ...
  if (application_tasks_allowed && !task_execution_allowed_) {
    ...
    WINBASE_DCHECK(RunLoop::IsNestedOnCurrentThread());
...
void MessageLoop::RunTask(PendingTask* pending_task) {
  WINBASE_DCHECK(task_execution_allowed_);
