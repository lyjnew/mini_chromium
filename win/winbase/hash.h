...
#ifndef WINLIB_WINBASE_HASH_H_
#define WINLIB_WINBASE_HASH_H_
...
WINBASE_EXPORT uint32_t Hash(const void* data, size_t length);
WINBASE_EXPORT uint32_t Hash(const std::string& str);
WINBASE_EXPORT uint32_t Hash(const string16& str);
...
//
// WARNING: This hash function should not be used for any cryptographic purpose.
WINBASE_EXPORT uint32_t PersistentHash(const void* data, size_t length);
WINBASE_EXPORT uint32_t PersistentHash(const std::string& str);

// Hash pairs of 32-bit or 64-bit numbers.
WINBASE_EXPORT size_t HashInts32(uint32_t value1, uint32_t value2);
WINBASE_EXPORT size_t HashInts64(uint64_t value1, uint64_t value2);
...
//   std::unordered_set<MyPair, winbase::IntPairHash<MyPair>> set;
template <typename T>
struct IntPairHash;
