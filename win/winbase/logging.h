...
#ifndef WINLIB_WINBASE_LOGGING_H_
#define WINLIB_WINBASE_LOGGING_H_
...
#include "winbase\strings\string_piece_forward.h"
...
//
//   WINBASE_LOG(INFO) << "Found " << num_cookies << " cookies";
//
// You can also do conditional logging:
//
//   WINBASE_LOG_IF(INFO, num_cookies > 10) << "Got lots of cookies";
//
...
//
//   WINBASE_DLOG(INFO) << "Found cookies";
//
//   WINBASE_DLOG_IF(INFO, num_cookies > 10) << "Got lots of cookies";
//
...
// We also have
//
//   WINBASE_LOG_ASSERT(assertion);
//   WINBASE_DLOG_ASSERT(assertion);
//
// which is syntactic sugar for {,D}LOG_IF(FATAL, assert fails) << assertion;
//
// There are "verbose level" logging macros.  They look like
//
//   WINBASE_VLOG(1) << "I'm printed when you run the program with --v=1 or more";
//   WINBASE_VLOG(2) << "I'm printed when you run the program with --v=2 or more";
//
// These always log at the INFO log level (when they log at all).
// The verbose logging can also be turned on module-by-module.  For instance,
//    --vmodule=profile=2,icon_loader=1,browser_*=3,*/chromeos/*=4 --v=0
// will cause:
//   a. WINBASE_VLOG(2) and lower messages to be printed from profile.{h,cc}
//   b. WINBASE_VLOG(1) and lower messages to be printed from icon_loader.{h,cc}
//   c. WINBASE_VLOG(3) and lower messages to be printed from files prefixed with
//      "browser"
//   d. WINBASE_VLOG(4) and lower messages to be printed from files under a
//     "chromeos" directory.
//   e. WINBASE_VLOG(0) and lower messages to be printed from elsewhere
//
...
//
// There's also WINBASE_VLOG_IS_ON(n) "verbose level" condition macro. 
// To be used as
//
...
// Lastly, there is:
//
//   WINBASE_PLOG(ERROR) << "Couldn't do foo";
//   WINBASE_DPLOG(ERROR) << "Couldn't do foo";
//   WINBASE_PLOG_IF(ERROR, cond) << "Couldn't do foo";
//   WINBASE_DPLOG_IF(ERROR, cond) << "Couldn't do foo";
//   WINBASE_PCHECK(condition) << "Couldn't do foo";
//   WINBASE_DPCHECK(condition) << "Couldn't do foo";
//
...
struct WINBASE_EXPORT LoggingSettings {
...
// or vice versa.
#if defined(NDEBUG)
#define WinBaseInitLoggingImpl BaseInitLoggingImpl_built_with_NDEBUG
#else
#define WinBaseInitLoggingImpl BaseInitLoggingImpl_built_without_NDEBUG
#endif

// Implementation of the InitLogging() method declared below.  We use a
// more-specific name so we can #define it above without affecting other code
// that has named stuff "InitLogging".
WINBASE_EXPORT bool WinBaseInitLoggingImpl(const LoggingSettings& settings);
...
inline bool InitLogging(const LoggingSettings& settings) {
  return WinBaseInitLoggingImpl(settings);
}
...
WINBASE_EXPORT void SetMinLogLevel(int level);

// Gets the current log level.
WINBASE_EXPORT int GetMinLogLevel();

// Used by LOG_IS_ON to lazy-evaluate stream arguments.
WINBASE_EXPORT bool ShouldCreateLogMessage(int severity);

// Gets the VLOG default verbosity level.
WINBASE_EXPORT int GetVlogVerbosity();

// Note that |N| is the size *with* the null terminator.
WINBASE_EXPORT int GetVlogLevelHelper(const char* file_start, size_t N);
...
// only.
WINBASE_EXPORT void SetLogItems(bool enable_process_id, bool enable_thread_id,
                                bool enable_timestamp, bool enable_tickcount);

// Sets whether or not you'd like to see fatal debug messages popped up in
// a dialog box or not.
// Dialogs are not shown by default.
WINBASE_EXPORT void SetShowErrorDialogs(bool enable_dialogs);
...
using LogAssertHandlerFunction =
    winbase::Callback<void(const char* file,
                           int line,
                           const winbase::StringPiece message,
                           const winbase::StringPiece stack_trace)>;

class WINBASE_EXPORT ScopedLogAssertHandler {
 public:
...
WINBASE_EXPORT void SetLogMessageHandler(LogMessageHandlerFunction handler);
WINBASE_EXPORT LogMessageHandlerFunction GetLogMessageHandler();
...
#define WINBASE_ANALYZER_ASSUME_TRUE(arg) \
  ::winbase::logging::AnalyzerAssumeTrue(!!(arg))
#define WINBASE_ANALYZER_SKIP_THIS_PATH() \
  static_cast<void>(::winbase::logging::AnalyzerNoReturn())
#define WINBASE_ANALYZER_ALLOW_UNUSED(var) static_cast<void>(var);

#else  // !defined(__clang_analyzer__)

#define WINBASE_ANALYZER_ASSUME_TRUE(arg) (arg)
#define WINBASE_ANALYZER_SKIP_THIS_PATH()
#define WINBASE_ANALYZER_ALLOW_UNUSED(var) static_cast<void>(var);
...
#define COMPACT_WINBASE_LOG_EX_INFO(ClassName, ...)             \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                               ::winbase::logging::LOG_INFO, ##__VA_ARGS__)
#define COMPACT_WINBASE_LOG_EX_WARNING(ClassName, ...)          \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                                ::winbase::logging::LOG_WARNING, ##__VA_ARGS__)
#define COMPACT_WINBASE_LOG_EX_ERROR(ClassName, ...)            \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                                ::winbase::logging::LOG_ERROR, ##__VA_ARGS__)
#define COMPACT_WINBASE_LOG_EX_FATAL(ClassName, ...)            \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                                ::winbase::logging::LOG_FATAL, ##__VA_ARGS__)
#define COMPACT_WINBASE_LOG_EX_DFATAL(ClassName, ...)           \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                                ::winbase::logging::LOG_DFATAL, ##__VA_ARGS__)
#define COMPACT_WINBASE_LOG_EX_DCHECK(ClassName, ...)           \
  ::winbase::logging::ClassName(__FILE__, __LINE__,             \
                                ::winbase::logging::LOG_DCHECK, ##__VA_ARGS__)

#define COMPACT_WINBASE_LOG_INFO    COMPACT_WINBASE_LOG_EX_INFO(LogMessage)
#define COMPACT_WINBASE_LOG_WARNING COMPACT_WINBASE_LOG_EX_WARNING(LogMessage)
#define COMPACT_WINBASE_LOG_ERROR   COMPACT_WINBASE_LOG_EX_ERROR(LogMessage)
#define COMPACT_WINBASE_LOG_FATAL   COMPACT_WINBASE_LOG_EX_FATAL(LogMessage)
...
// may be slow.
#define WINBASE_VLOG_IS_ON(verboselevel)       \
  ((verboselevel) <= ::winbase::logging::GetVlogLevel(__FILE__))

// Helper macro which avoids evaluating the arguments to a stream if
// the condition doesn't hold. Condition is evaluated once and only once.
#define WINBASE_LAZY_STREAM(stream, condition)                           \
...
#define WINBASE_LOG_STREAM(severity) COMPACT_WINBASE_LOG_##severity.stream()

#define WINBASE_LOG(severity) \
  WINBASE_LAZY_STREAM(WINBASE_LOG_STREAM(severity), WINBASE_LOG_IS_ON(severity))
#define WINBASE_LOG_IF(severity, condition) \
...
#define WINBASE_VLOG(verbose_level)                       \
...
#define WINBASE_VPLOG_STREAM(verbose_level)                                    \
  ::winbase::logging::Win32ErrorLogMessage(__FILE__, __LINE__, -verbose_level, \
    ::winbase::logging::GetLastSystemErrorCode()).stream()

#define WINBASE_VPLOG(verbose_level)                       \
  WINBASE_LAZY_STREAM(WINBASE_VPLOG_STREAM(verbose_level), \
                      WINBASE_VLOG_IS_ON(verbose_level))

#define WINBASE_VPLOG_IF(verbose_level, condition)         \
  WINBASE_LAZY_STREAM(WINBASE_VPLOG_STREAM(verbose_level), \
                      WINBASE_VLOG_IS_ON(verbose_level) && (condition))
...
#define WINBASE_PLOG_STREAM(severity) \
...
WINBASE_EXPORT extern std::ostream* g_swallow_stream;
...
// behavior.
#define WINBASE_EAT_STREAM_PARAMETERS                \
  true ? (void)0                                     \
...
#define WINBASE_WRAPPED_TRAP_SEQUENCE() \
  do {                                  \
    [] { WINBASE_TRAP_SEQUENCE(); }();  \
  } while (false)
#else
#define WINBASE_WRAPPED_TRAP_SEQUENCE() WINBASE_TRAP_SEQUENCE()
#endif

#define WINBASE_IMMEDIATE_CRASH()    \
  ({                                 \
    WINBASE_WRAPPED_TRAP_SEQUENCE(); \
...
#else
#define WINBASE_IMMEDIATE_CRASH() __debugbreak()
#endif  // __clang__
...
// WINBASE_CHECK dies with a fatal error if condition is not true.  It is *not*
// controlled by NDEBUG, so the check will be executed regardless of
// compilation mode.
//
// We make sure WINBASE_CHECK et al. always evaluates their arguments, as
// doing WINBASE_CHECK(FunctionWithSideEffect()) is a common idiom.
...
#define WINBASE_CHECK(condition) \
...
#define WINBASE_CHECK_OP(name, op, val1, val2) WINBASE_CHECK((val1)op(val2))
...
#define WINBASE_CHECK(condition)                            \
  __analysis_assume(!!(condition)),                         \
      WINBASE_LAZY_STREAM(WINBASE_LOG_STREAM(FATAL), false) \
          << "Check failed: " #condition ". "

#define WINBASE_PCHECK(condition)                            \
  __analysis_assume(!!(condition)),                          \
      WINBASE_LAZY_STREAM(WINBASE_PLOG_STREAM(FATAL), false) \
          << "Check failed: " #condition ". "
...
#define WINBASE_CHECK(condition)                                               \
  WINBASE_LAZY_STREAM(                                                         \
      ::winbase::logging::LogMessage(__FILE__, __LINE__, #condition).stream(), \
                                     !WINBASE_ANALYZER_ASSUME_TRUE(condition))

#define WINBASE_PCHECK(condition)                               \
  WINBASE_LAZY_STREAM(WINBASE_PLOG_STREAM(FATAL),               \
                      !WINBASE_ANALYZER_ASSUME_TRUE(condition)) \
      << "Check failed: " #condition ". "
...
#define WINBASE_CHECK_OP(name, op, val1, val2)                                 \
  ...
  if (::winbase::logging::CheckOpResult true_if_passed =                       \
   ...
   ;                                                                           \
    ...
    ::winbase::logging::LogMessage(__FILE__, __LINE__,                         \
...
inline typename std::enable_if<
    winbase::internal::SupportsOstreamOperator<const T&>::value &&
...
WINBASE_EXPORT void MakeCheckOpValueString(std::ostream* os, std::nullptr_t p);
...
// in logging.cc.
extern template WINBASE_EXPORT std::string* MakeCheckOpString<int, int>(
    const int&, const int&, const char* names);
extern template WINBASE_EXPORT
...
#define WINBASE_CHECK_EQ(val1, val2) WINBASE_CHECK_OP(EQ, ==, val1, val2)
#define WINBASE_CHECK_NE(val1, val2) WINBASE_CHECK_OP(NE, !=, val1, val2)
#define WINBASE_CHECK_LE(val1, val2) WINBASE_CHECK_OP(LE, <=, val1, val2)
#define WINBASE_CHECK_LT(val1, val2) WINBASE_CHECK_OP(LT, < , val1, val2)
#define WINBASE_CHECK_GE(val1, val2) WINBASE_CHECK_OP(GE, >=, val1, val2)
#define WINBASE_CHECK_GT(val1, val2) WINBASE_CHECK_OP(GT, > , val1, val2)

#if defined(NDEBUG) && !defined(WINBASE_DCHECK_ALWAYS_ON)
#define WINBASE_DCHECK_IS_ON() 0
#else
#define WINBASE_DCHECK_IS_ON() 1
#endif

// Definitions for WINBASE_DLOG et al.

#if WINBASE_DCHECK_IS_ON()

#define WINBASE_DLOG_IS_ON(severity) \
  WINBASE_LOG_IS_ON(severity)
#define WINBASE_DLOG_IF(severity, condition) \
  WINBASE_LOG_IF(severity, condition)
#define WINBASE_DLOG_ASSERT(condition) \
  WINBASE_LOG_ASSERT(condition)
...
#define WINBASE_DLOG_IF(severity, condition) WINBASE_EAT_STREAM_PARAMETERS
#define WINBASE_DLOG_ASSERT(condition) WINBASE_EAT_STREAM_PARAMETERS
#define WINBASE_DPLOG_IF(severity, condition) WINBASE_EAT_STREAM_PARAMETERS
#define WINBASE_DVLOG_IF(verboselevel, condition) WINBASE_EAT_STREAM_PARAMETERS
#define WINBASE_DVPLOG_IF(verboselevel, condition) WINBASE_EAT_STREAM_PARAMETERS

#endif  // WINBASE_DCHECK_IS_ON()

#define WINBASE_DLOG(severity)                      \
  WINBASE_LAZY_STREAM(WINBASE_LOG_STREAM(severity), \
...
WINBASE_EXPORT extern LogSeverity LOG_DCHECK;
#else
...
#define WINBASE_DCHECK(condition)                               \
  WINBASE_LAZY_STREAM(WINBASE_LOG_STREAM(DCHECK),               \
                      !WINBASE_ANALYZER_ASSUME_TRUE(condition)) \
      << "Check failed: " #condition ". "
#define WINBASE_DPCHECK(condition)                              \
  WINBASE_LAZY_STREAM(WINBASE_PLOG_STREAM(DCHECK),              \
                      !WINBASE_ANALYZER_ASSUME_TRUE(condition)) \
      << "Check failed: " #condition ". "

#else  // WINBASE_DCHECK_IS_ON()

#define WINBASE_DCHECK(condition) WINBASE_EAT_STREAM_PARAMETERS << !(condition)
#define WINBASE_DPCHECK(condition) WINBASE_EAT_STREAM_PARAMETERS << !(condition)
...
#define WINBASE_DCHECK_OP(name, op, val1, val2)                        \
  ...
  if (::winbase::logging::CheckOpResult true_if_passed =               \
   ...
   ;                                                                   \
    ...
    ::winbase::logging::LogMessage(                                    \
          __FILE__, __LINE__, ::winbase::logging::LOG_DCHECK,          \
                          true_if_passed.message()).stream()
...
#define WINBASE_DCHECK_OP(name, op, val1, val2)         \
  WINBASE_EAT_STREAM_PARAMETERS <<                      \
      (::winbase::logging::MakeCheckOpValueString(      \
           ::winbase::logging::g_swallow_stream, val1), \
           ::winbase::logging::MakeCheckOpValueString(  \
           ::winbase::logging::g_swallow_stream, val2), \
           (val1)op(val2))
...
#define WINBASE_DCHECK_EQ(val1, val2) WINBASE_DCHECK_OP(EQ, ==, val1, val2)
#define WINBASE_DCHECK_NE(val1, val2) WINBASE_DCHECK_OP(NE, !=, val1, val2)
#define WINBASE_DCHECK_LE(val1, val2) WINBASE_DCHECK_OP(LE, <=, val1, val2)
#define WINBASE_DCHECK_LT(val1, val2) WINBASE_DCHECK_OP(LT, < , val1, val2)
#define WINBASE_DCHECK_GE(val1, val2) WINBASE_DCHECK_OP(GE, >=, val1, val2)
#define WINBASE_DCHECK_GT(val1, val2) WINBASE_DCHECK_OP(GT, > , val1, val2)

#define WINBASE_NOTREACHED() WINBASE_DCHECK(false)

// Redefine the standard assert to use our nice log files
#undef assert
#define assert(x) WINBASE_DLOG_ASSERT(x)
...
WINBASE_EXPORT SystemErrorCode GetLastSystemErrorCode();
WINBASE_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code);

// Appends a formatted system message of the GetLastError() type.
class WINBASE_EXPORT Win32ErrorLogMessage {
 public:
...
//       after this call.
WINBASE_EXPORT void CloseLogFile();

// Async signal safe logging mechanism.
WINBASE_EXPORT void RawLog(int level, const char* message);

#define WINBASE_RAW_LOG(level, message) \
  ::winbase::logging::RawLog(::winbase::logging::LOG_##level, message)

#define WINBASE_RAW_CHECK(condition)                            \
  do {                                                          \
    if (!(condition))                                           \
      ::winbase::logging::RawLog(::winbase::logging::LOG_FATAL, \
                        "Check failed: " #condition "\n");      \
  } while (0)

// Returns true if logging to file is enabled.
WINBASE_EXPORT bool IsLoggingToFileEnabled();

// Returns the default log file path.
WINBASE_EXPORT std::wstring GetLogFileFullPath();
...
// operators.
WINBASE_EXPORT std::ostream& operator<<(std::ostream& out, const wchar_t* wstr);
...
#define WINBASE_NOTIMPLEMENTED_MSG "Not implemented reached in " << __PRETTY_FUNCTION__
#else
#define WINBASE_NOTIMPLEMENTED_MSG "NOT IMPLEMENTED"
#endif

#define WINBASE_NOTIMPLEMENTED() \
  WINBASE_LOG(ERROR) << WINBASE_NOTIMPLEMENTED_MSG
#define WINBASE_NOTIMPLEMENTED_LOG_ONCE()                              \
  do {                                                                 \
    static bool logged_once = false;                                   \
    WINBASE_LOG_IF(ERROR, !logged_once) << WINBASE_NOTIMPLEMENTED_MSG; \
    logged_once = true;                                                \
  } while (0);                                                         \
  WINBASE_EAT_STREAM_PARAMETERS

#endif  // WINLIB_WINBASE_LOGGING_H_