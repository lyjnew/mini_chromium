...
AtExitManager::~AtExitManager() {
  if (!g_top_manager) {
    WINBASE_NOTREACHED() << "Tried to ~AtExitManager without an AtExitManager";
    return;
  }
  WINBASE_DCHECK_EQ(this, g_top_manager);
...
void AtExitManager::RegisterCallback(AtExitCallbackType func, void* param) {
  WINBASE_DCHECK(func);
  RegisterTask(winbase::Bind(func, param));
}

// static
void AtExitManager::RegisterTask(winbase::Closure task) {
  if (!g_top_manager) {
    WINBASE_NOTREACHED()
  ...
  WINBASE_DCHECK(!g_top_manager->processing_callbacks_);
...
void AtExitManager::ProcessCallbacksNow() {
  ...
  winbase::stack<winbase::Closure> tasks;
  {
    AutoLock lock(g_top_manager->lock_);
  ...
  while (!tasks.empty()) {
    winbase::Closure task = tasks.top();
    task.Run();
    tasks.pop();
  }

  // Expect that all callbacks have been run.
  WINBASE_DCHECK(g_top_manager->stack_.empty());
}
...
AtExitManager::AtExitManager(bool shadow)
    : processing_callbacks_(false), next_manager_(g_top_manager) {
  WINBASE_DCHECK(shadow || !g_top_manager);
  g_top_manager = this;
}
