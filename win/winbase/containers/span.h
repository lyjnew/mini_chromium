...
#ifndef WINLIB_WINBASE_CONTAINERS_SPAN_H_
#define WINLIB_WINBASE_CONTAINERS_SPAN_H_
...
using ContainerHasConvertibleData = IsLegalDataConversion<
    std::remove_pointer_t<decltype(winbase::data(std::declval<Container>()))>,
    T>;
...
class span {
 public:
  ...
  constexpr span(T* data, size_t size) noexcept : data_(data), size_(size) {
    WINBASE_CHECK(Extent == dynamic_extent || Extent == size);
  }
  ...
  constexpr span<T, Count> first() const noexcept {
    ...
    WINBASE_CHECK(Extent != dynamic_extent || Count <= size());
    return {data(), Count};
  }
  ...
  subspan() const noexcept {
    ...
    WINBASE_CHECK(Extent != dynamic_extent || Offset <= size());
    WINBASE_CHECK(Extent != dynamic_extent || Count == dynamic_extent ||
                  Count <= size() - Offset);
  ...
  constexpr span<T, dynamic_extent> subspan(
    ...
    // Note: CHECK_LE is not constexpr, hence regular CHECK must be used.
    WINBASE_CHECK(offset <= size());
    WINBASE_CHECK(count == dynamic_extent || count <= size() - offset);
    return {data() + offset, count != dynamic_extent ? count : size() - offset};
  }
...
#endif  // WINLIB_WINBASE_CONTAINERS_SPAN_H_