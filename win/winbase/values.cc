...
#include "winbase\json\json_writer.h"
...
///#include "winbase\trace_event\memory_usage_estimator.h"
...
Value::Value(std::string&& in_string) noexcept
  ...
  WINBASE_DCHECK(IsStringUTF8(string_value_));
}
...
bool Value::GetBool() const {
  WINBASE_CHECK(is_bool());
  return bool_value_;
}

int Value::GetInt() const {
  WINBASE_CHECK(is_int());
  return int_value_;
}
...
const Value* Value::FindPath(std::initializer_list<StringPiece> path) const {
  WINBASE_DCHECK_GE(path.size(), 2u) << "Use FindKey() for a path of length 1.";
...
const Value* Value::FindPathOfType(std::initializer_list<StringPiece> path,
                                   Type type) const {
  WINBASE_DCHECK_GE(path.size(), 2u)
      << "Use FindKeyOfType() for a path of length 1.";
...
Value* Value::SetPath(std::initializer_list<StringPiece> path, Value value) {
  WINBASE_DCHECK_GE(path.size(), 2u) << "Use SetKey() for a path of length 1.";
  return SetPath(make_span(path.begin(), path.size()), std::move(value));
}

Value* Value::SetPath(span<const StringPiece> path, Value value) {
  WINBASE_DCHECK_NE(path.begin(), path.end());  // Can't be empty path.
...
bool Value::RemovePath(std::initializer_list<StringPiece> path) {
  WINBASE_DCHECK_GE(path.size(), 2u)
      << "Use RemoveKey() for a path of length 1.";
...
bool DictionaryValue::HasKey(StringPiece key) const {
  WINBASE_DCHECK(IsStringUTF8(key));
  auto current_entry = dict_.find(key);
  WINBASE_DCHECK((current_entry == dict_.end()) || current_entry->second);
...
Value* DictionaryValue::Set(StringPiece path, std::unique_ptr<Value> in_value) {
  WINBASE_DCHECK(IsStringUTF8(path));
  WINBASE_DCHECK(in_value);
...
void DictionaryValue::MergeDictionary(const DictionaryValue* dictionary) {
  WINBASE_CHECK(dictionary->is_dict());
