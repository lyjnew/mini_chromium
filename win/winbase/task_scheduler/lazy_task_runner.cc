...
#include "winbase\task_scheduler\lazy_task_runner.h"
...
void LazyTaskRunner<TaskRunnerType, com_sta>::Reset() {
  ...
  WINBASE_DCHECK_NE(state, kLazyInstanceStateCreating) 
      << "Race: all threads should be "
...
LazyTaskRunner<SequencedTaskRunner, false>::Create() {
  ...
  WINBASE_DCHECK_EQ(thread_mode_, SingleThreadTaskRunnerThreadMode::SHARED);
...
ScopedLazyTaskRunnerListForTesting::ScopedLazyTaskRunnerListForTesting() {
  WINBASE_DCHECK(!g_scoped_lazy_task_runner_list_for_testing);
