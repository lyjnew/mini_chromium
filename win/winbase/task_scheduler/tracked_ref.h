...
#ifndef WINLIB_WINBASE_TASK_SCHEDULER_TRACKED_REF_H_
#define WINLIB_WINBASE_TASK_SCHEDULER_TRACKED_REF_H_
 ...
 public:
  ...
  ~TrackedRef() {
    if (factory_ && !factory_->live_tracked_refs_.Decrement()) {
      WINBASE_DCHECK(factory_->ready_to_destroy_);
      WINBASE_DCHECK(!factory_->ready_to_destroy_->IsSignaled());
...
class TrackedRefFactory {
 public:
  ...
  TrackedRefFactory(const TrackedRefFactory<T>&) = delete;
  TrackedRefFactory<T>& operator=(const TrackedRefFactory<T>&) = delete;
  ...
  TrackedRef<T> GetTrackedRef() {
    ...
    WINBASE_DCHECK(!live_tracked_refs_.IsZero());
...
#endif  // WINLIB_WINBASE_TASK_SCHEDULER_TRACKED_REF_H_