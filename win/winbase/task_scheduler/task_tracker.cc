...
void TaskTracker::Shutdown() {
  PerformShutdown();
  WINBASE_DCHECK(IsShutdownComplete());
...
void TaskTracker::FlushAsyncForTesting(OnceClosure flush_callback) {
  WINBASE_DCHECK(flush_callback);
  {
    AutoSchedulerLock auto_lock(flush_lock_);
    WINBASE_DCHECK(!flush_callback_for_testing_)
...
void TaskTracker::RunOrSkipTask(Task task,
...

  {
    ...
    WINBASE_DCHECK(sequence_token.IsValid());
    ...
    WINBASE_DCHECK(!task.sequenced_task_runner_ref ||
...
TaskTracker::GetPreemptedBackgroundSequenceToScheduleLockRequired() {
  ...
  WINBASE_DCHECK(!preempted_background_sequences_.empty());

  ++num_scheduled_background_sequences_;
  WINBASE_DCHECK_LE(num_scheduled_background_sequences_,
                    max_num_scheduled_background_sequences_);
...
void TaskTracker::SchedulePreemptedBackgroundSequence(
  ...
  WINBASE_DCHECK(sequence_to_schedule.observer);
...
bool TaskTracker::BeforeRunTask(TaskShutdownBehavior shutdown_behavior) {
  switch (shutdown_behavior) {
    case TaskShutdownBehavior::BLOCK_SHUTDOWN: {
      ...
      // task was posted.
      WINBASE_DCHECK(state_->AreTasksBlockingShutdown());
      ...
      // did.
      WINBASE_DCHECK(!state_->HasShutdownStarted() || !IsShutdownComplete());
...
void TaskTracker::DecrementNumIncompleteUndelayedTasks() {
  ...
  WINBASE_DCHECK_GE(new_num_incomplete_undelayed_tasks, 0);
