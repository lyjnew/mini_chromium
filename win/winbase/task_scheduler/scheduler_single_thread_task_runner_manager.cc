...
class SchedulerWorkerDelegate : public SchedulerWorker::Delegate {
 public:
  ...
  SchedulerWorkerDelegate(const SchedulerWorkerDelegate&) = delete;
  SchedulerWorkerDelegate& operator=(const SchedulerWorkerDelegate&) = delete;

  void set_worker(SchedulerWorker* worker) {
    WINBASE_DCHECK(!worker_);
    worker_ = worker;
  }
  ...
  void ReEnqueueSequence(scoped_refptr<Sequence> sequence) override {
    ...
    WINBASE_DCHECK_EQ(sequence, sequence_);
    WINBASE_DCHECK(!has_work_);
    has_work_ = true;
  }
...
class SchedulerWorkerCOMDelegate : public SchedulerWorkerDelegate {
 public:
  ...
  SchedulerWorkerCOMDelegate(
      const SchedulerWorkerCOMDelegate&) = delete;
  SchedulerWorkerCOMDelegate& operator=(
      const SchedulerWorkerCOMDelegate&) = delete;
  ~SchedulerWorkerCOMDelegate() override { 
    WINBASE_DCHECK(!scoped_com_initializer_); 
  }
  ...
  void WaitForWork(WaitableEvent* wake_up_event) override {
    ...
    const DWORD milliseconds_wait =
        sleep_time.is_max() ? 
            INFINITE : static_cast<DWORD>(sleep_time.InMilliseconds());
 ...
 private:
  scoped_refptr<Sequence> GetWorkFromWindowsMessageQueue() {
    ...
    if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE) != FALSE) {
      Task pump_message_task(WINBASE_FROM_HERE,
                             Bind(
                                 [](MSG msg) {
...
class SchedulerSingleThreadTaskRunnerManager::SchedulerSingleThreadTaskRunner
 ...
 public:
  ...
  SchedulerSingleThreadTaskRunner(
    ...
    WINBASE_DCHECK(outer_);
    WINBASE_DCHECK(worker_);
  }

  SchedulerSingleThreadTaskRunner(
      const SchedulerSingleThreadTaskRunner&) = delete;
  SchedulerSingleThreadTaskRunner& operator=(
      const SchedulerSingleThreadTaskRunner&) = delete;
...
SchedulerSingleThreadTaskRunnerManager::SchedulerSingleThreadTaskRunnerManager(
  ...
  WINBASE_DCHECK(delayed_task_manager_);

  static_assert(array_size(shared_com_scheduler_workers_) ==
                    array_size(shared_scheduler_workers_),
                "The size of |shared_com_scheduler_workers_| must match "
                "|shared_scheduler_workers_|");
  static_assert(array_size(shared_com_scheduler_workers_[0]) ==
                    array_size(shared_scheduler_workers_[0]),
                "The size of |shared_com_scheduler_workers_| must match "
                "|shared_scheduler_workers_|");

  WINBASE_DCHECK(!g_manager_is_alive);
...
SchedulerSingleThreadTaskRunnerManager::CreateTaskRunnerWithTraitsImpl(
    const TaskTraits& traits,
    SingleThreadTaskRunnerThreadMode thread_mode) {
  WINBASE_DCHECK(thread_mode != SingleThreadTaskRunnerThreadMode::SHARED ||
...
void SchedulerSingleThreadTaskRunnerManager::UnregisterSchedulerWorker(
  ...
  scoped_refptr<SchedulerWorker> worker_to_destroy;
  {
    ...
    WINBASE_DCHECK(worker_iter != workers_.end());
...
void SchedulerSingleThreadTaskRunnerManager::ReleaseSharedSchedulerWorkers() {
  ...
  {
    AutoSchedulerLock auto_lock(lock_);
    for (size_t i = 0; i < array_size(shared_scheduler_workers_); ++i) {
      for (size_t j = 0; j < array_size(shared_scheduler_workers_[i]); ++j) {
  ...
  for (size_t i = 0; i < array_size(local_shared_scheduler_workers); ++i) {
    for (size_t j = 0; j < array_size(local_shared_scheduler_workers[i]); ++j) {
