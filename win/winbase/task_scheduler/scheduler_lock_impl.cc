...
class SafeAcquisitionTracker {
 public:
  ...
  void RegisterLock(
    ...
    WINBASE_DCHECK_NE(lock, predecessor) << "Reentrant locks are unsupported.";
  ...
  void RecordRelease(const SchedulerLockImpl* const lock) {
    ...
    WINBASE_DCHECK(iter_at_lock != acquired_locks->end());
 ...
 private:
  ...
  void AssertSafeAcquire(const SchedulerLockImpl* const lock) {
    ...
    WINBASE_DCHECK_EQ(acquired_locks->back(), allowed_predecessor);
  }
  ...
  void AssertSafePredecessor(const SchedulerLockImpl* lock) const {
    ...
    if (predecessor) {
      WINBASE_DCHECK(allowed_predecessor_map_.find(predecessor) !=
