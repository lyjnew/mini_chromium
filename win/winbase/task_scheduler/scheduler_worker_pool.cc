...
LazyInstance<ThreadLocalPointer<const SchedulerWorkerPool>>::Leaky
    tls_current_worker_pool = WINBASE_LAZY_INSTANCE_INITIALIZER;
...
class SchedulerParallelTaskRunner : public TaskRunner {
 public:
  ...
  SchedulerParallelTaskRunner(const TaskTraits& traits,
    ...
    WINBASE_DCHECK(worker_pool_);
  }

  SchedulerParallelTaskRunner(
      const SchedulerParallelTaskRunner&) = delete;
  SchedulerParallelTaskRunner& operator=(
      const SchedulerParallelTaskRunner&) = delete;
...
class SchedulerSequencedTaskRunner : public SequencedTaskRunner {
 public:
  ...
  SchedulerSequencedTaskRunner(
      const SchedulerSequencedTaskRunner&) = delete;
  SchedulerSequencedTaskRunner& operator=(
      const SchedulerSequencedTaskRunner&) = delete;
  ...
  bool PostNonNestableDelayedTask(const Location& from_here,
                                  OnceClosure closure,
                                  winbase::TimeDelta delay) override {
...
SchedulerWorkerPool::~SchedulerWorkerPool() {
  --g_active_pools_count;
  WINBASE_DCHECK_GE(g_active_pools_count, 0);
}

void SchedulerWorkerPool::BindToCurrentThread() {
  WINBASE_DCHECK(!GetCurrentWorkerPool());
...
void SchedulerWorkerPool::PostTaskWithSequenceNow(
  ...
  // in the past).
  WINBASE_DCHECK_LE(task.delayed_run_time, TimeTicks::Now());
