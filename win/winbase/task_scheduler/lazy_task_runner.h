...
#ifndef WINBASE_WINBASE_TASK_SCHEDULER_LAZY_TASK_RUNNER_H_
#define WINBASE_WINBASE_TASK_SCHEDULER_LAZY_TASK_RUNNER_H_
...
#include "winbase\lazy_instance_helpers.h"
...
// winbase::Create(Sequenced|SingleThread|COMSTA)TaskRunnerWithTraits() as a member
// on an object accessible by all PostTask() call sites.
//
// Example usage 1:
//
// namespace {
// winbase::LazySequencedTaskRunner g_sequenced_task_runner =
//     WINBASE_LAZY_SEQUENCED_TASK_RUNNER_INITIALIZER(
//         base::TaskTraits(winbase::MayBlock(),
//                          winbase::TaskPriority::USER_VISIBLE));
// }  // namespace
//
// void SequencedFunction() {
//   // Different invocations of this function post to the same
//   // MayBlock() SequencedTaskRunner.
//   g_sequenced_task_runner.Get()->PostTask(WINBASE_FROM_HERE, 
//                                           winbase::BindOnce(...));
// }
//
// Example usage 2:
//
// namespace {
// winbase::LazySequencedTaskRunner g_sequenced_task_task_runner =
//     WINBASE_LAZY_SEQUENCED_TASK_RUNNER_INITIALIZER({base::MayBlock()});
// }  // namespace
//
...
class WINBASE_EXPORT LazyTaskRunner;
}  // namespace internal
...
#define WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL2(a, b) a##b
#define WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL(a, b) \
  WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL2(a, b)
...
#define WINBASE_LAZY_SEQUENCED_TASK_RUNNER_INITIALIZER(traits)                 \
  winbase::LazySequencedTaskRunner::CreateInternal(traits);                    \
  ALLOW_UNUSED_TYPE constexpr winbase::TaskTraits                              \
      WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL(kVerifyTraitsAreConstexpr, \
                                            __LINE__) = traits
...
#define WINBASE_LAZY_SINGLE_THREAD_TASK_RUNNER_INITIALIZER(traits,             \
                                                           thread_mode)        \
  winbase::LazySingleThreadTaskRunner::CreateInternal(traits, thread_mode);    \
  ALLOW_UNUSED_TYPE constexpr winbase::TaskTraits                              \
      WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL(kVerifyTraitsAreConstexpr, \
                                            __LINE__) = traits;                \
  ALLOW_UNUSED_TYPE constexpr winbase::SingleThreadTaskRunnerThreadMode        \
      WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL(                           \
          kVerifyThreadModeIsConstexpr, __LINE__) = thread_mode
...
#define WINBASE_LAZY_COM_STA_TASK_RUNNER_INITIALIZER(traits, thread_mode)      \
  winbase::LazyCOMSTATaskRunner::CreateInternal(traits, thread_mode);          \
  ALLOW_UNUSED_TYPE constexpr winbase::TaskTraits                              \
      WINBASE_LAZY_TASK_RUNNER_CONCATENATE_INTERNAL(kVerifyTraitsAreConstexpr, \
                                            __LINE__) = traits;                \
  ALLOW_UNUSED_TYPE constexpr winbase::SingleThreadTaskRunnerThreadMode        \
...
class WINBASE_EXPORT ScopedLazyTaskRunnerListForTesting {
 public:
...
#endif  // WINBASE_WINBASE_TASK_SCHEDULER_LAZY_TASK_RUNNER_H_