...
#include "winbase\json\json_parser.h"
...
class StackMarker {
 public:
  StackMarker(int max_depth, int* depth)
    ...
    ++(*depth_);
    WINBASE_DCHECK_LE(*depth_, max_depth_);
  }
...
JSONParser::JSONParser(int options, int max_depth)
    : options_(options),
      ...
      error_line_(0),
      error_column_(0) {
  WINBASE_CHECK_LE(max_depth, JSONReader::kStackMaxDepth);
}
...
Optional<Value> JSONParser::Parse(StringPiece input) {
  ...
  if (!winbase::IsValueInRangeForNumericType<int32_t>(input.length())) {
...
void JSONParser::StringBuilder::Append(uint32_t point) {
  WINBASE_DCHECK(IsValidCharacter(point));

  if (point < kExtendedASCIIStart && !string_) {
    WINBASE_DCHECK_EQ(static_cast<char>(point), pos_[length_]);
    ++length_;
  } else {
    Convert();
...
const char* JSONParser::pos() {
  WINBASE_CHECK_LE(static_cast<size_t>(index_), input_.length());
...
bool JSONParser::DecodeUTF16(uint32_t* out_code_point) {
  ...
  if (CBU16_IS_SURROGATE(code_unit16_high)) {
    ...
    *out_code_point = code_point;
    ...
    // Not a surrogate.
    WINBASE_DCHECK(CBU16_IS_SINGLE(code_unit16_high));
