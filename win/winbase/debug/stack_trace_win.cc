...
bool InitializeSymbols() {
  ...
  if (!SymInitialize(GetCurrentProcess(), NULL, TRUE)) {
    ...
    // process).
    WINBASE_DLOG(ERROR) << "SymInitialize failed: " << g_init_error;
    return false;
  }
  ...
  if (!SymGetSearchPathW(GetCurrentProcess(),
                         ...
                         kSymbolsArraySize)) {
    g_init_error = GetLastError();
    WINBASE_DLOG(WARNING) << "SymGetSearchPath failed: " << g_init_error;
    return false;
  }
  ...
  if (!SymSetSearchPathW(GetCurrentProcess(), new_path.c_str())) {
    g_init_error = GetLastError();
    WINBASE_DLOG(WARNING) << "SymSetSearchPath failed." << g_init_error;
    return false;
  }
...
void StackTrace::InitTrace(const CONTEXT* context_record) {
  ...
  while (StackWalk64(machine_type,
                     ...
                     &SymGetModuleBase64,
                     NULL) &&
         count_ < array_size(trace_)) {
    trace_[count_++] = reinterpret_cast<void*>(stack_frame.AddrPC.Offset);
  }

  for (size_t i = count_; i < array_size(trace_); ++i)
    trace_[i] = NULL;
}
