...
#include "winbase\win\scoped_process_information.h"
#include "winbase\win\startup_information.h"
...
bool GetAppOutputInternal(const StringPiece16& cl,
  ...
  if (!CreatePipe(&out_read, &out_write, &sa_attr, 0)) {
    WINBASE_NOTREACHED() << "Failed to create pipe";
    return false;
  }
  ...
  if (!SetHandleInformation(out_read, HANDLE_FLAG_INHERIT, 0)) {
    WINBASE_NOTREACHED() << "Failed to disabled pipe inheritance";
    return false;
  }
  ...
  if (!CreateProcess(nullptr, &writable_command_line_string[0], nullptr,
    ...
    WINBASE_NOTREACHED() << "Failed to start process";
    return false;
  }

  winbase::win::ScopedProcessInformation proc_info(temp_process_info);
  ...
  winbase::TerminationStatus status = GetTerminationStatus(
      proc_info.process_handle(), exit_code);
  ///winbase::debug::GlobalActivityTracker::RecordProcessExitIfEnabled(
  ///    proc_info.process_id(), *exit_code);
  return status != winbase::TERMINATION_STATUS_PROCESS_CRASHED &&
         status != winbase::TERMINATION_STATUS_ABNORMAL_TERMINATION;
}
...
Process LaunchProcess(const string16& cmdline,
  ...
  if (!options.handles_to_inherit.empty()) {
    WINBASE_DCHECK_EQ(options.inherit_mode, LaunchOptions::Inherit::kSpecific);

    if (options.handles_to_inherit.size() >
        std::numeric_limits<DWORD>::max() / sizeof(HANDLE)) {
      WINBASE_DLOG(ERROR) << "Too many handles to inherit.";
      return Process();
    }
  ...
  if (options.stdin_handle || options.stdout_handle || options.stderr_handle) {
    WINBASE_DCHECK(inherit_handles);
    WINBASE_DCHECK(options.stdin_handle);
    WINBASE_DCHECK(options.stdout_handle);
    WINBASE_DCHECK(options.stderr_handle);
  ...
  WINBASE_DCHECK(!(flags & CREATE_SUSPENDED))
  ...
  if (options.as_user) {
    ...
    if (!launched) {
      WINBASE_DPLOG(ERROR) << "Command line:" << std::endl
                           << UTF16ToUTF8(cmdline)
                           << std::endl;
      return Process();
    }
  ...
  winbase::win::ScopedProcessInformation process_info(temp_process_info);

  if (options.job_handle &&
      !AssignProcessToJobObject(options.job_handle,
                                process_info.process_handle())) {
    WINBASE_DPLOG(ERROR) << "Could not AssignProcessToObject";
  ...
  ///winbase::debug::GlobalActivityTracker::RecordProcessLaunchIfEnabled(
