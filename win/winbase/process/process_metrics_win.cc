...
#include "winbase\process\process_metrics_iocounters.h"
...
class WorkingSetInformationBuffer {
 public:
  ...
  WorkingSetInformationBuffer(
      const WorkingSetInformationBuffer&) = delete;
  WorkingSetInformationBuffer& operator=(
      const WorkingSetInformationBuffer&) = delete;
...
size_t GetSystemCommitCharge() {
  ...
  if (!GetPerformanceInfo(&info, sizeof(info))) {
    WINBASE_DLOG(ERROR) << "Failed to fetch internal performance info.";
    return 0;
  }
