...
#include "winbase\process\process_info.h"
...
HANDLE GetCurrentProcessToken() {
  ...
  WINBASE_DCHECK(process_token != NULL &&
                 process_token != INVALID_HANDLE_VALUE);
...
bool IsCurrentProcessElevated() {
  ...
  if (!GetTokenInformation(process_token, TokenElevation, &elevation,
    ...
    WINBASE_PLOG(ERROR) << "GetTokenInformation() failed";
    return false;
  }
