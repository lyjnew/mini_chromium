...
#include "winbase\strings\string_split.h"
#include "winbase\strings\string_tokenizer.h"
...
size_t switch_prefix_count = array_size(kSwitchPrefixes);
...
void CommandLine::set_slash_is_not_a_switch() {
  ...
  WINBASE_DCHECK_EQ(
      wcscmp(kSwitchPrefixes[array_size(kSwitchPrefixes) - 1], L"/"), 0);
  switch_prefix_count = array_size(kSwitchPrefixes) - 1;
}

// static
void CommandLine::InitUsingArgvForTesting(int argc, const char* const* argv) {
  WINBASE_DCHECK(!current_process_commandline_);
  current_process_commandline_ = new CommandLine(NO_PROGRAM);
  // On Windows we need to convert the command line arguments to string16.
  winbase::CommandLine::StringVector argv_vector;
...
bool CommandLine::HasSwitch(
    const winbase::StringPiece& switch_string) const {
  WINBASE_DCHECK_EQ(ToLowerASCII(switch_string), switch_string);
  return ContainsKey(switches_, switch_string);
}

bool CommandLine::HasSwitch(const char switch_constant[]) const {
  return HasSwitch(winbase::StringPiece(switch_constant));
}
...
std::string CommandLine::GetSwitchValueASCII(
  ...
  if (!IsStringASCII(value)) {
    WINBASE_DLOG(WARNING) << "Value of switch (" << switch_string
                          << ") must be ASCII.";
    return std::string();
  }
...
void CommandLine::AppendArg(const std::string& value) {
  WINBASE_DCHECK(IsStringUTF8(value));
...
void CommandLine::PrependWrapper(const CommandLine::StringType& wrapper) {
  ...
  CommandLineTokenizer tokenizer(wrapper, WINBASE_FILE_PATH_LITERAL(" "));
  tokenizer.set_quote_chars(WINBASE_FILE_PATH_LITERAL("'\""));
...
CommandLine::StringType CommandLine::GetCommandLineStringInternal(
  ...
  if (!params.empty()) {
    string.append(StringType(WINBASE_FILE_PATH_LITERAL(" ")));
...
CommandLine::StringType CommandLine::GetArgumentsStringInternal(
  ...
  for (size_t i = 1; i < argv_.size(); ++i) {
    ...
    parse_switches &= arg != kSwitchTerminator;
    if (i > 1)
      params.append(StringType(WINBASE_FILE_PATH_LITERAL(" ")));
    if (parse_switches && IsSwitch(arg, &switch_string, &switch_value)) {
