...
///#include "winbase\debug\profiler.h"
...
void PlatformThread::Join(PlatformThreadHandle thread_handle) {
  WINBASE_DCHECK(thread_handle.platform_handle());
  ...
  WINBASE_CHECK_EQ(WAIT_OBJECT_0,
...
void PlatformThread::SetCurrentThreadPriority(ThreadPriority priority) {
  ...
  switch (priority) {
    ...
    case ThreadPriority::REALTIME_AUDIO:
      ...
      break;
    default:
      WINBASE_NOTREACHED() << "Unknown priority.";
      break;
  }
  WINBASE_DCHECK_NE(desired_priority, THREAD_PRIORITY_ERROR_RETURN);
  ...
  const BOOL success =
      ...
      ::SetThreadPriority(PlatformThread::CurrentHandle().platform_handle(),
                          desired_priority);
  WINBASE_DPLOG_IF(ERROR, !success)
...
ThreadPriority PlatformThread::GetCurrentThreadPriority() {
  ...
  switch (priority) {
    ...
    case THREAD_PRIORITY_ERROR_RETURN:
      WINBASE_DPCHECK(false) << "GetThreadPriority error";
      FALLTHROUGH;
    default:
      WINBASE_NOTREACHED() << "Unexpected priority: " << priority;
      return ThreadPriority::NORMAL;
  }
}
