...
#include "winbase\threading\watchdog.h"
...
StaticData* GetStaticData() {
  static winbase::NoDestructor<StaticData> static_data;
...
void Watchdog::Alarm() {
  WINBASE_DVLOG(1) << "Watchdog alarmed for " << thread_watched_name_;
}
...
void Watchdog::ThreadDelegate::ThreadMain() {
  ...
  while (1) {
    ...
    WINBASE_DCHECK(ARMED == watchdog_->state_);
...
void Watchdog::ThreadDelegate::SetThreadName() const {
  ...
  WINBASE_DVLOG(1) << "Watchdog active: " << name;
}
