...
bool PostTaskAndReplyImpl::PostTaskAndReply(const Location& from_here,
                                            OnceClosure task,
                                            OnceClosure reply) {
  WINBASE_DCHECK(task) << from_here.ToString();
  WINBASE_DCHECK(reply) << from_here.ToString();
