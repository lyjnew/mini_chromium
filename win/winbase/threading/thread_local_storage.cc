...
using winbase::internal::PlatformThreadLocalStorage;
...
winbase::subtle::Atomic32 g_native_tls_key =
...
struct TlsMetadata {
  TlsStatus status;
  winbase::ThreadLocalStorage::TLSDestructorFunc destructor;
  uint32_t version;
};
...
winbase::Lock* GetTLSMetadataLock() {
  static auto* lock = new winbase::Lock();
  return lock;
}
...
TlsVectorEntry* ConstructTlsVector() {
  ...
  if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
    WINBASE_CHECK(PlatformThreadLocalStorage::AllocTLS(&key));
    ...
    if (PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES !=
        static_cast<PlatformThreadLocalStorage::TLSKey>(
            winbase::subtle::NoBarrier_CompareAndSwap(
                &g_native_tls_key,
      ...
      key = winbase::subtle::NoBarrier_Load(&g_native_tls_key);
    }
  }
  WINBASE_CHECK_EQ(PlatformThreadLocalStorage::GetTLSValue(key), kUninitialized);
...
void OnThreadExitInternal(TlsVectorEntry* tls_data) {
  ...
  {
    winbase::AutoLock auto_lock(*GetTLSMetadataLock());
  ...
  while (need_to_scan_destructors) {
    ...
    if (--remaining_attempts <= 0) {
      WINBASE_NOTREACHED();  // Destructors might not have been called.
      break;
    }
  }
...
void PlatformThreadLocalStorage::OnThreadExit() {
  ...
  WINBASE_DCHECK_NE(tls_data, kDestroyed);
...
void ThreadLocalStorage::Slot::Initialize(TLSDestructorFunc destructor) {
  ...
  WINBASE_CHECK_NE(slot_, kInvalidSlotValue);
  WINBASE_CHECK_LT(slot_, kThreadLocalStorageSize);
}

void ThreadLocalStorage::Slot::Free() {
  WINBASE_DCHECK_NE(slot_, kInvalidSlotValue);
  WINBASE_DCHECK_LT(slot_, kThreadLocalStorageSize);
  {
    winbase::AutoLock auto_lock(*GetTLSMetadataLock());
