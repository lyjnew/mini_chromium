...
void TruncateUTF8ToByteSize(const std::string& input,
  ...
  while (char_index >= 0) {
    ...
    winbase_icu::UChar32 code_point = 0;
...
bool IsUnicodeWhitespace(wchar_t c) {
  // kWhitespaceUTF16 is a NULL-terminated string
  for (const wchar_t* cur = kWhitespaceUTF16; *cur; ++cur) {
    if (*cur == c)
      return true;
  }
  return false;
...
string16 FormatBytesUnlocalized(int64_t bytes) {
  ...
  if (bytes != 0 && dimension > 0 && unit_amount < 100) {
    winbase::snprintf(buf, array_size(buf), "%.1lf%s", unit_amount,
                      kByteStringsUnlocalized[dimension]);
  } else {
    winbase::snprintf(buf, array_size(buf), "%.0lf%s", unit_amount,
                      kByteStringsUnlocalized[dimension]);
  }
