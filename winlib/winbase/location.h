...
#ifndef WINLIB_WINBASE_LOCATION_H_
#define WINLIB_WINBASE_LOCATION_H_
...
WINBASE_EXPORT const void* GetProgramCounter();

// The macros defined here will expand to the current function.
#if defined(WINBASE_ENABLE_LOCATION_SOURCE)

// Full source information should be included.
#define WINBASE_FROM_HERE WINBASE_FROM_HERE_WITH_EXPLICIT_FUNCTION(__func__)
#define WINBASE_FROM_HERE_WITH_EXPLICIT_FUNCTION(function_name) \
  ::winbase::Location::CreateFromHere(function_name, __FILE__, __LINE__)

#else

// TODO(http://crbug.com/760702) remove the __FILE__ argument from these calls.
#define WINBASE_FROM_HERE ::winbase::Location::CreateFromHere(__FILE__)
#define WINBASE_FROM_HERE_WITH_EXPLICIT_FUNCTION(function_name) \
  ::winbase::Location::CreateFromHere(function_name, __FILE__, -1)
...
template <>
struct hash<::winbase::Location> {
  std::size_t operator()(const ::winbase::Location& loc) const {
    const void* program_counter = loc.program_counter();
    return ::winbase::Hash(&program_counter, sizeof(void*));
  }
};

}  // namespace std

#endif  // WINLIB_WINBASE_LOCATION_H_