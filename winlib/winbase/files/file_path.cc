...
const FilePath::CharType kStringTerminator = WINBASE_FILE_PATH_LITERAL('\0');
...
#if defined(WINBASE_FILE_PATH_USES_DRIVE_LETTERS)
...
#endif  // WINBASE_FILE_PATH_USES_DRIVE_LETTERS
...
#endif  // defined(WINBASE_FILE_PATH_USES_DRIVE_LETTERS)
...
#else  // WINBASE_FILE_PATH_USES_DRIVE_LETTERS
...
#else  // defined(WINBASE_FILE_PATH_USES_DRIVE_LETTERS)
...
FilePath FilePath::InsertBeforeExtensionASCII(StringPiece suffix)
    const {
  WINBASE_DCHECK(IsStringASCII(suffix));
...
bool FilePath::MatchesExtension(StringPieceType extension) const {
  WINBASE_DCHECK(extension.empty() || extension[0] == kExtensionSeparator);
...
FilePath FilePath::Append(StringPieceType component) const {
  ...
  WINBASE_DCHECK(!IsPathAbsolute(appended));
...
FilePath FilePath::AppendASCII(StringPiece component) const {
  WINBASE_DCHECK(winbase::IsStringASCII(component));
...
bool FilePath::ReferencesParent() const {
  ...
  for (; it != components.end(); ++it) {
    ...
    if (component.find_first_not_of(WINBASE_FILE_PATH_LITERAL(". \n\r\t")) ==
            std::string::npos &&
...
std::string FilePath::MaybeAsASCII() const {
  if (winbase::IsStringASCII(path_))
