...
bool PickleIterator::ReadLong(long* result) {
  ...
  *result = winbase::checked_cast<long>(result_int64);
  return true;
}
...
Pickle::Pickle(int header_size)
    : header_(nullptr),
      ...
      capacity_after_header_(0),
      write_offset_(0) {
  WINBASE_DCHECK_GE(static_cast<size_t>(header_size), sizeof(Header));
  WINBASE_DCHECK_LE(header_size, kPayloadUnit);
  Resize(kPayloadUnit);
  header_->payload_size = 0;
}
...
void Pickle::Reserve(size_t length) {
  ...
  WINBASE_DCHECK_GE(data_len, length);
#ifdef ARCH_CPU_64_BITS
  WINBASE_DCHECK_LE(data_len, std::numeric_limits<uint32_t>::max());
#endif
  WINBASE_DCHECK_LE(write_offset_,
                    std::numeric_limits<uint32_t>::max() - data_len);
...
bool Pickle::ReadAttachment(winbase::PickleIterator* iter,
...
void Pickle::Resize(size_t new_capacity) {
  WINBASE_CHECK_NE(capacity_after_header_, kCapacityReadOnly);
...
bool Pickle::PeekNext(size_t header_size,
                      ...
                      const char* end,
                      size_t* pickle_size) {
  WINBASE_DCHECK_EQ(header_size, bits::Align(header_size, sizeof(uint32_t)));
  WINBASE_DCHECK_GE(header_size, sizeof(Header));
  WINBASE_DCHECK_LE(header_size, static_cast<size_t>(kPayloadUnit));
...
inline void* Pickle::ClaimUninitializedBytesInternal(size_t length) {
  WINBASE_DCHECK_NE(kCapacityReadOnly, capacity_after_header_)
