...
#ifndef WINLIB_WINBASE_OPTIONAL_H_
#define WINLIB_WINBASE_OPTIONAL_H_
...
struct IsSwappableImpl {
  ...
  template <typename T>
  static auto Check(int) -> decltype(swap(std::declval<T>(), 
...
// winbase::Optional is a Chromium version of the C++17 optional class:
...
template <class T>
struct hash<winbase::Optional<T>> {
  size_t operator()(const winbase::Optional<T>& opt) const {
    return opt == winbase::nullopt ? 0 : std::hash<T>()(*opt);
  }
};

}  // namespace std

#endif  // WINLIB_WINBASE_OPTIONAL_H_